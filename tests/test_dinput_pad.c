...
int main(void) {
  ...
  CHECK(strcmp(dinput_pad_persistent_id(0), dinput_pad_persistent_id(1)) != 0);
...
#define TRIG_DOWN (32767)
  ...
  {
    static const struct {
      int sdl_button, di_button;
      const char *nm;
    } B[] = {{0, 0, "A"},  {1, 1, "B"},  {2, 2, "X"},    {3, 3, "Y"},
             {8, 4, "LB"}, {9, 5, "RB"}, {4, 6, "Back"}, {5, 7, "Start"},
             {6, 8, "LS"}, {7, 9, "RS"}};
    int i, j;
  ...
  /* ---- the d-pad as a POV, in hundredths of a degree ---- */
  {
    static const struct {
      unsigned char hat;
      uint32_t pov;
    } H[] = {{SDL_HAT_UP, 0},       {SDL_HAT_RIGHTUP, 4500},
