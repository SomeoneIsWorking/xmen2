...
static const TestCommand cleanup_commands[] = {
    COMMAND_CAMERA_RESET,    COMMAND_WAIT_CLEANUP,     COMMAND_AI_ENABLED,
    COMMAND_CONTROL_RELEASE, COMMAND_CLEANUP_COMPLETE,
};
...
step_owned_fiber(void *context, X2CutsceneSequence sequence,
  ...
  fiber->cursor++;
  if (command == COMMAND_ROOT_COMPLETE || command == COMMAND_SPAWN_COMPLETE ||
      command == COMMAND_WALK_COMPLETE || command == COMMAND_CLEANUP_COMPLETE) {
    fiber->active = 0;
