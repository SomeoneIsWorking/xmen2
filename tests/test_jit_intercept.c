 ...
 * native-override check on the current guest call frame being present. The old
 ...
 *
 * The address check must hold regardless of frame-stack depth, and regardless
 * of the frame the caller hands in -- x86port passes the run's own frame as
 * the per-run pointer, so this test supplies it directly.
 */
