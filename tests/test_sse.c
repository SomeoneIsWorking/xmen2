...
static void report(const char *what, V128 host, V128 model, V128 a, V128 b) {
  ...
  printf("  FAIL %-10s a=%016llx%016llx b=%016llx%016llx\n"
         "        host  %016llx%016llx\n"
         "        model %016llx%016llx\n",
         what, (unsigned long long)a.q[1], (unsigned long long)a.q[0],
...
HOST_BIN(h_addps, "addps")
HOST_BIN(h_subps, "subps") HOST_BIN(h_mulps, "mulps") HOST_BIN(h_divps, "divps")
    HOST_BIN(h_minps, "minps") HOST_BIN(h_maxps, "maxps") HOST_BIN(
        h_andps, "andps") HOST_BIN(h_andnps, "andnps") HOST_BIN(h_orps, "orps")
        HOST_BIN(h_xorps, "xorps") HOST_BIN(h_addss, "addss")
            HOST_BIN(h_subss, "subss") HOST_BIN(h_mulss, "mulss")
                HOST_BIN(h_divss, "divss") HOST_BIN(h_minss, "minss")
                    HOST_BIN(h_maxss, "maxss") HOST_BIN(h_unpcklps, "unpcklps")
                        HOST_BIN(h_unpckhps, "unpckhps")
                            HOST_BIN(h_movhlps, "movhlps") HOST_BIN(h_movlhps,
                                                                    "movlhps")
                                HOST_BIN(h_sqrtps, "sqrtps") HOST_BIN(h_rcpps,
                                                                      "rcpps")
                                    HOST_BIN(h_rsqrtps, "rsqrtps")
                                        HOST_BIN(h_sqrtss, "sqrtss")
                                            HOST_BIN(h_rcpss, "rcpss")
...
static V128 h_cmpps(V128 a, V128 b, int pred) {
  ...
  switch (pred) {
    CMP1(0)
    CMP1(1) CMP1(2) CMP1(3) CMP1(4) CMP1(5) CMP1(6) CMP1(7) default : out = a;
    break;
  }
  return out;
}
...
static V128 h_cmpss(V128 a, V128 b, int pred) {
  ...
  switch (pred) {
    CMPS1(0)
    CMPS1(1) CMPS1(2) CMPS1(3) CMPS1(4) CMPS1(5) CMPS1(6) CMPS1(7) default
        : out = a;
    break;
  }
  return out;
...
static V128 h_shufps(V128 a, V128 b, int imm) {
  V128 out;
  switch (imm) {
    SH64(0) SH64(64) SH64(128) SH64(192) default : out = a;
    break;
  }
  return out;
}
...
#undef CHK
      ...
      for (imm = 0; imm < 256; imm++) {
        m = a;
        sse_shufps(m.q, b.q, (unsigned)imm);
      ...
      }
      {
        ...
        if ((hf & CMP) != (mf & CMP)) {
          printf("  FAIL COMISS lane0 %08x vs %08x: flags 0x%03x "
                 "want 0x%03x\n",
                 (uint32_t)a.q[0], (uint32_t)b.q[0], mf & CMP, hf & CMP);
          fails++;
        }
      }
    }
