...
static int close_enough(float a, float b) { return fabsf(a - b) < 0.001f; }
...
static int test_pretransformed_bounds(void) {
  const float vertices[][4] = {{10.0f, 70.0f, 0.25f, 1.0f},
...
static int test_indexed_transformed_bounds(void) {
  const float vertices[][3] = {{-1.0f, -1.0f, 0.1f},
                               {1.0f, -1.0f, 0.2f},
                               {1.0f, 1.0f, 0.3f},
                               {-1.0f, 1.0f, 0.4f}};
  ...
  return !(
      ...
      bounds.behind == 0 && close_enough(bounds.min_x, 0.0f) &&
      close_enough(bounds.max_x, 800.0f) && close_enough(bounds.min_y, 0.0f) &&
      close_enough(bounds.max_y, 600.0f) && close_enough(bounds.min_z, 0.3f) &&
...
static int exercise_runtime_writer_when_armed(void) {
  ...
  const float vertices[][4] = {{10.0f, 20.0f, 0.25f, 1.0f},
