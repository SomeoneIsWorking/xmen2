/*
 * Unit tests for the engine leaf-thunk fast path (src/native/engine_leaf_thunks.c).
 */
...
static void test_ftol_leaf(void) {
  ...
  int handled = engine_leaf_thunk_dispatch(&cpu);
  CHECK(handled == 1, "ftol not handled by leaf thunk dispatch");
...
static void test_stricmp_leaf(void) {
  ...
  int handled = engine_leaf_thunk_dispatch(&cpu);
  CHECK(handled == 1, "stricmp not handled by leaf thunk dispatch");
  ...
  WR32(STACK + 8u, STR2);
  cpu.eip = stricmp_addr;
  handled = engine_leaf_thunk_dispatch(&cpu);
  CHECK(handled == 1, "stricmp unequal not handled");
  CHECK((int32_t)cpu.reg[kX86pEax] < 0, "stricmp Alpha vs Beta should be negative");
...
static void test_qpc_and_qpf_leaf(void) {
  ...
  uint32_t qpf_addr = x86_native_thunk("KERNEL32.DLL", "QueryPerformanceFrequency");
  cpu.eip = qpf_addr;
  int handled = engine_leaf_thunk_dispatch(&cpu);
  CHECK(handled == 1, "QPF not handled");
  CHECK(cpu.reg[kX86pEax] == 1u, "QPF return not 1");
  CHECK(cpu.reg[kX86pEsp] == STACK + 8u, "QPF did not pop 8 bytes (__stdcall 1 arg)");
  ...
  WR32(STACK, RET_ADDR);
  WR32(STACK + 4u, OUT_BUF);
  uint32_t qpc_addr = x86_native_thunk("KERNEL32.DLL", "QueryPerformanceCounter");
  cpu.eip = qpc_addr;
  handled = engine_leaf_thunk_dispatch(&cpu);
...
static void test_enable_disable(void) {
  ...
  CHECK(engine_leaf_thunks_is_enabled() == 1, "should default to enabled");
  CHECK(engine_leaf_thunk_dispatch(&cpu) == 1, "should handle when enabled");

  engine_leaf_thunks_enable(0);
  CHECK(engine_leaf_thunks_is_enabled() == 0, "should report disabled");
  CHECK(engine_leaf_thunk_dispatch(&cpu) == 0, "should return 0 when disabled");

  engine_leaf_thunks_enable(1);
  cpu.eip = toupper_addr;
  CHECK(engine_leaf_thunks_is_enabled() == 1, "should report re-enabled");
  CHECK(engine_leaf_thunk_dispatch(&cpu) == 1, "should handle when re-enabled");
}
