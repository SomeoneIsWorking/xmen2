...
static void test_skip_stops_and_suppresses_dialogue(void) {
  ...
  CHECK(
      after.active_voice_stops == before.active_voice_stops + 1u &&
          after.suppressed_response_starts ==
              before.suppressed_response_starts + 1u &&
          after.suppressed_line_starts == before.suppressed_line_starts + 1u &&
          after.skip_presentation_starts == before.skip_presentation_starts,
...
static void test_skip_scope_suppresses_adjacent_line(void) {
  ...
  CHECK(after.suppressed_line_starts == before.suppressed_line_starts + 1u &&
            after.skip_presentation_starts == before.skip_presentation_starts,
