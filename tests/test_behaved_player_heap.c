...
static X86Module module = {
    .name = "XMen2.exe", .base = &mapped_exe, .preferred = EXE_BASE};
static struct {
  const char *module;
  uint32_t ep;
  x86_override_fn fn;
...
static uint32_t scheduler(void) { return manager_address + SCHEDULER_OFFSET; }
...
static void test_arbitrary_remove_bubbles_down(void) {
  static const float deadlines[] = {1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f};
  static const uint32_t slots[] = {0u, 1u, 2u, 3u, 4u, 5u, 6u};
  Pair before[7], selected;
  unsigned index;
  CPU cpu = fresh_cpu();
