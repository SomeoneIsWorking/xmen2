...
{
    char path[PATH_MAX];
    if (!start_dir[0] || !test_dir[0]) return;
    chdir(start_dir);
...
int main(int argc, char **argv)
{
    ...
    int rc;
    if (argc != 2) return 2;
    if (!getcwd(start_dir, sizeof start_dir)) return 2;
    snprintf(test_dir, sizeof test_dir, "env-file-test-%ld", (long)getpid());
    if (mkdir(test_dir, 0700) != 0) return 2;
    atexit(cleanup);
