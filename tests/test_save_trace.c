...
static void disabled_is_an_explicit_refusal(void) {
  ...
  CHECK(save_trace_mark(&trace, SAVE_TRACE_MENU_BUILD, SAVE_TRACE_ANSWER_YES,
                        "main") == SAVE_TRACE_REFUSED_DISABLED);
...
static void records_both_answers_and_manager_fields(void) {
  ...
  CHECK(save_trace_mark(&trace, SAVE_TRACE_MAP_00484CE0, SAVE_TRACE_ANSWER_YES,
                        "act0/tutorial") == SAVE_TRACE_RECORDED);
  CHECK(save_trace_mark(&trace, SAVE_TRACE_MAP_00484CE0, SAVE_TRACE_ANSWER_NO,
                        "no-save") == SAVE_TRACE_RECORDED);
  CHECK(save_trace_load_manager(&trace, SAVE_TRACE_ANSWER_YES, 5, 37, 2, 7,
                                0x12345678) == SAVE_TRACE_RECORDED);
...
static void invalid_and_capacity_refusals_are_visible(void) {
  ...
  CHECK(save_trace_mark(&trace, SAVE_TRACE_POINT_COUNT,
                        SAVE_TRACE_ANSWER_UNKNOWN,
                        NULL) == SAVE_TRACE_REFUSED_INVALID);
  ...
  CHECK(save_trace_mark(&trace, SAVE_TRACE_EXTRACTION_SAVE_COMMAND,
                        ...
                        long_label) == SAVE_TRACE_RECORDED);
  CHECK(save_trace_event_at(&trace, 0, &event));
