...
static void coordinate_contract(void) {
  ...
  CHECK(x2_win32_mouse_map_point(960, 540, 1920, 1080, 1280, 720, &x, &y));
  CHECK(x == 640 && y == 360);
  CHECK(x2_win32_mouse_pack_point(640, 360) == 0x01680280u);

  CHECK(x2_win32_mouse_unmap_point(400, 300, 1280, 720, 800, 600, &host_x,
                                   &host_y));
  CHECK(host_x == 640 && host_y == 360);
  CHECK(x2_win32_mouse_map_point(host_x, host_y, 1280, 720, 800, 600, &x, &y));
  CHECK(x == 400 && y == 300);
  CHECK(
      x2_win32_mouse_unmap_point(0, 0, 1280, 720, 800, 600, &host_x, &host_y));
  CHECK(host_x == 160 && host_y == 0);
  CHECK(x2_win32_mouse_unmap_point(799, 599, 1280, 720, 800, 600, &host_x,
                                   &host_y));
  CHECK(host_x == 1119 && host_y == 719);
  CHECK(x2_win32_mouse_map_point(host_x, host_y, 1280, 720, 800, 600, &x, &y));
  CHECK(x == 799 && y == 599);

  CHECK(x2_win32_mouse_unmap_point(0, 0, 800, 800, 800, 600, &host_x, &host_y));
  CHECK(host_x == 0 && host_y == 100);
  CHECK(x2_win32_mouse_unmap_point(799, 599, 800, 800, 800, 600, &host_x,
                                   &host_y));
  ...
  CHECK(x2_win32_mouse_unmap_point(-20, 900, 1280, 720, 800, 600, &host_x,
                                   &host_y));
  ...
  CHECK(!x2_win32_mouse_unmap_point(0, 0, 0, 600, 800, 600, &host_x, &host_y));
  CHECK(!x2_win32_mouse_unmap_point(0, 0, 800, 600, 800, 32769, &host_x,
                                    &host_y));
}
...
static void mouse_message_contract(void) {
  ...
  CHECK(x2_win32_mouse_motion(&mouse, 4u, 10, 20, 110, 220, 30u, X2_MK_LBUTTON,
                              X2_MK_SHIFT));
  CHECK(x2_win32_mouse_button(&mouse, 4u, X2_WIN32_MOUSE_RIGHT, 1, 11, 21, 111,
                              221, 31u, X2_MK_CONTROL));
  CHECK(x2_win32_mouse_button(&mouse, 4u, X2_WIN32_MOUSE_RIGHT, 0, 12, 22, 112,
                              222, 32u, 0u));
  ...
  CHECK(found.wparam == (X2_MK_LBUTTON | X2_MK_RBUTTON | X2_MK_CONTROL));

  CHECK(x2_win32_message_take(&mouse, 0u, 0u, 0u, 1, &found));
  CHECK(found.message == X2_WM_RBUTTONUP);
  CHECK(found.wparam == X2_MK_LBUTTON);

  CHECK(x2_win32_mouse_motion(&mouse, 4u, 20, 30, 120, 230, 40u, 0u, 0u));
  CHECK(x2_win32_mouse_motion(&mouse, 4u, 21, 31, 121, 231, 41u, 0u, 0u));
  CHECK(mouse.message_count == 1u);
  CHECK(x2_win32_message_take(&mouse, 0u, 0u, 0u, 1, &found));
  CHECK(found.message == X2_WM_MOUSEMOVE);
  CHECK(found.lparam == 0x001f0015u);

  CHECK(x2_win32_mouse_button(&mouse, 4u, X2_WIN32_MOUSE_LEFT, 1, 0, 0, 0, 0,
                              0u, 0u));
  CHECK(x2_win32_message_take(&mouse, 0u, 0u, 0u, 1, &found));
