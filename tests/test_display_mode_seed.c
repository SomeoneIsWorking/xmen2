...
int main(void) {
  ...
  CHECK(x2_display_mode_seed_plan(NULL, 1280, 720, value, (int)sizeof value) ==
        1);
  ...
  CHECK(x2_display_mode_seed_plan("", 2560, 1440, value, (int)sizeof value) ==
        1);
  CHECK(strcmp(value, "2560x1440") == 0);

  /* Dimensions that cannot be an output size are refused, not formatted. */
  CHECK(x2_display_mode_seed_plan(NULL, 0, 720, value, (int)sizeof value) == 0);
  CHECK(x2_display_mode_seed_plan(NULL, 1280, 0, value, (int)sizeof value) ==
        0);
  CHECK(x2_display_mode_seed_plan(NULL, 99999, 720, value, (int)sizeof value) ==
        0);
