...
static X2FmvDrainResult receive_delayed(void *userdata)
{
    ...
    if (fake->queued == fake->capacity) return X2_FMV_DRAIN_OUTPUT_BLOCKED;
    if (!fake->delayed) return X2_FMV_DRAIN_COMPLETE;
    fake->delayed--;
    fake->queued++;
...
{
    FakeDecoder *fake = (FakeDecoder *)userdata;
    if (!fake->tail) return X2_FMV_DRAIN_COMPLETE;
    fake->tail--;
    fake->tail_outputs++;
    return X2_FMV_DRAIN_PROGRESS;
}
...
int main(void)
{
    X2FmvDecoderDrainOps video_ops = {
        accept_flush, receive_delayed, NULL
    };
    X2FmvDecoderDrainOps audio_ops = {
        accept_flush, receive_delayed, flush_tail
    };
