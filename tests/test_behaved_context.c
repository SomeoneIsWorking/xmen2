...
static void check_arguments(CPU *cpu, uint32_t expected_count,
                            uint32_t first)
{
    ...
    if (expected_count)
        check(RD32(list) == first,
              "handler received the wrong first argument");
}
                         ...
                         uint32_t callee_pop_bytes)
{
    if (target == FN_MANAGER) {
        ...
        check(cpu->ecx == CONTEXT + 8u,
              "context value lookup used the wrong map");
        cpu->eax = RD32(RD32(cpu->esp)) == 0x33u
            ? 1u : INVALID_INDEX;
...
static void set_node(uint32_t node, uint32_t handler, uint32_t next,
                     uint32_t alternate, uint8_t suspend,
                     uint8_t conditional)
{
...
int main(void)
{
    ...
    check(assignments == 1u,
          "named result was not assigned exactly once");
    check(releases == 3u,
