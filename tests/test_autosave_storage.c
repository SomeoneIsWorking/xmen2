...
static int temporary_count(const char *directory)
{
    ...
    while ((entry = readdir(dir)) != NULL)
        if (!strncmp(entry->d_name, ".autosave.save.tmp.", 19u)) count++;
    closedir(dir);
    return count;
}
...
int main(void)
{
    ...
    CHECK(result[128] == sizeof payload && result[129] == 0
          && result[130] == 0 && result[131] == 0);
    CHECK(!memcmp(result + 132u, payload, sizeof payload));
    CHECK(temporary_count(directory) == 0);

    CHECK(!x2_autosave_storage_publish(NULL, header, payload,
                                       sizeof payload,
                                       X2_AUTOSAVE_FAULT_NONE));
