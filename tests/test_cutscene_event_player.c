...
static uint32_t slot_bit(uint32_t slot)
{
    return 1u << (slot % 32u);
}
...
static int queue_event(uint32_t slot, float deadline)
{
    ...
    while (index != 0u) {
        ...
        if (!(pair_deadline(pair) < pair_deadline(parent_pair))) break;
...
void x86_register_override(const char *module, uint32_t ep,
                           x86_override_fn fn)
{
    if (registration_count >= 2u) abort();
...
static x86_override_fn registered_fn(uint32_t ep)
{
    ...
    for (index = 0; index < registration_count; ++index)
        if (registered[index].ep == ep) return registered[index].fn;
    return NULL;
}
...
static void test_strict_ordinary_pump(void)
{
    ...
    WR32(cpu.esp + 4u, float_bits(5.0f));
    registered_fn(FN_EVENT_PUMP)(&cpu);
    CHECK(RD32(OWNER + HEAP_COUNT) == 2u,
...
{
    return left.deadline_bits == right.deadline_bits &&
                                  ...
                                  Pair removed)
{
    ...
    for (index = 0; index < before_count; ++index) {
        int found = 0;
        if (pairs_equal(before[index], removed)) continue;
        for (other = 0; other < after_count; ++other)
            if (!matched[other] && pairs_equal(before[index],
                                               read_pair(other))) {
                matched[other] = 1u;
                found = 1;
                break;
...
static void test_arbitrary_heap_removal(void)
{
    ...
    for (index = 0; index < 7u; ++index) before[index] = read_pair(index);
    removed = before[4];
