...
static void check(int condition, const char *expression, int line)
{
    ...
    if (condition) return;
    fprintf(stderr,
            "test_d3d8_live_resolution: check %d failed at line %d: %s\n",
...
void gpu_frame_viewport(int x, int y, int width, int height,
...
int main(void)
{
    ...
    CHECK(backbuffer.pitch == 1920 * 4 &&
          backbuffer.size == 1920 * 1080 * 4);
