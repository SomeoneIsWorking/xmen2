...
int main(void)
{
    ...
    if (!h) { printf("cannot load DLL -- tested NOTHING\n"); return 2; }
    g_imgbase = (uint32_t)(uintptr_t)h;
    orig = (find_t)GetProcAddress(h,
        "?find@?$igTObjectList@VigController@Display@Gap@@@Core@Gap@@QBEHPBVigController@Display@3@H@Z");
    if (!orig) { printf("symbol not found -- tested NOTHING\n"); return 2; }

    for (i = 0; i < 8; i++) elems[i] = (void *)(uintptr_t)(0x1000 + i);
