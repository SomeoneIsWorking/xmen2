...
int main(void) {
  ...
  if (!getenv("X2_NORELOC")) {
    void *squat =
        VirtualAlloc((LPVOID)0x10000000, 0x100000, MEM_RESERVE, PAGE_NOACCESS);
    if (!squat)
  ...
  if (!h) {
    fprintf(stderr,
            "difftest: cannot load libIGDisplay_orig.dll (err %lu) "
            "-- tested NOTHING\n",
  ...
  if (!stack) {
    fprintf(stderr, "difftest: no stack\n");
    return 2;
  }
  ...
  if (!reg_o || !reg_r) {
    fprintf(stderr, "difftest: no regions\n");
    return 2;
  }
  ...
  { /* Imports must resolve for real: functions that call into libIGCore are
    ...
    if (bad) {
      printf("-- %d imports unresolved; tested NOTHING\n", bad);
      return 2;
    }
  }
  vf = fopen("verified.eps", "w");
  if (!vf)
    printf("-- WARNING: cannot write verified.eps\n");
  ...
  for (ci = 0; ci < NCASES; ci++) {
    ...
    for (t = 0; t < TRIALS; t++) {
      ...
      if (((got ^ want) & CASES[ci].mask) != 0) {
        ...
        unsigned off = 0;
        while (off < REGION && reg_o[off] == reg_r[off])
          off++;
        if (mism < 3)
          printf("  MEMDIFF %s arg=0x%08x first differing byte at "
                 "this%+d: orig=%02x recomp=%02x\n",
                 CASES[ci].label, arg, (int)off - OBJ_OFF, reg_o[off],
                 reg_r[off]);
        mism++;
      }
    }
