...
The in-repo registries answer what is known, trusted, and still next. Consult
them at the START of a task; update them in the SAME commit that changes a
subsystem.
...
scratch/build-native/x2native                           # the LIVE SDL3 GPU + D3D8 product target
scratch/build-native/x2native --d3d8                   # explicit spelling of that renderer route
./run.sh                                               # the same product target, building first if needed
```
...
`./run.sh` defaults to the current **native SDL3 GPU product**; optional
`RUN_ARGS` extend that route rather than replacing its required renderer. The
Wine paths keep their own names. `stock` is the untouched control and `wine`
keeps the hosted-recompiler path independently testable.
...
- **`src/display/`, `src/core/`, `src/app/`** — hand-written native code: the
  SDL3 controller backend and the IGB asset readers with their viewers. Port
  path is SDL3; the asset viewers stay SDL2 deliberately (separate binaries,
  no two-SDLs hazard).
