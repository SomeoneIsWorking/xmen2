...
static void capture_reason(char *why, int whyn, const char *reason) {
  if (why && whyn > 0)
    snprintf(why, (size_t)whyn, "%s", reason);
}
...
static int capture_dimensions(uint32_t width, uint32_t height, uint32_t *bytes,
                              char *why, int whyn) {
  size_t need;

  if (!width || !height) {
    capture_reason(why, whyn, "the final frame has a zero width or height");
    return 0;
  }
  ...
  if (need > GPU_CAPTURE_MAX_BYTES) {
    ...
    snprintf(reason, sizeof reason,
             ...
             "%u bytes",
             width, height, need, GPU_CAPTURE_MAX_BYTES);
...
int gpu_capture_result(const unsigned char **bgra, uint32_t *width,
  ...
  if (height)
    *height = 0;
  if (g_capture_state == CAPTURE_PENDING || g_capture_state == CAPTURE_RECORDED)
    return 0;
  if (g_capture_state == CAPTURE_FAILED) {
    capture_reason(why, whyn,
                   g_capture_failure[0] ? g_capture_failure
  ...
  if (bgra)
    *bgra = g_capture_pixels;
  if (width)
    *width = g_capture_width;
  if (height)
    *height = g_capture_height;
  return 1;
}
...
static int capture_reserve_pixels(uint32_t width, uint32_t height) {
  ...
  if ((size_t)bytes > g_capture_capacity) {
    ...
    if (!larger) {
      snprintf(reason, sizeof reason,
               "could not allocate the bounded %u-byte BGRA screenshot", bytes);
      capture_fail(reason);
      return 0;
    }
...
SDL_GPUTexture *gpu_capture_frame_target(SDL_GPUDevice *device,
  ...
  if (g_capture_state != CAPTURE_PENDING)
    return fallback;
  if (!device ||
      !capture_dimensions(width, height, &bytes, reason, sizeof reason)) {
    capture_fail(device ? reason
                        : "the GPU device disappeared before "
                          "the screenshot frame began");
    return fallback;
  }
  if (g_capture_texture && (g_capture_texture_width != width ||
...
int gpu_capture_frame_record(SDL_GPUDevice *device,
  ...
  if (!capture_reserve_pixels(width, height))
    return 0;
...
void gpu_capture_frame(int headless, unsigned long frame, uint32_t width,
  ...
  if (min_draws && gpu_frame_draws_so_far() < min_draws)
    return;
  if (require_vs && !gpu_frame_had_programmable())
    return;
  if (min_draws)
    busy_written++;
  ...
  if (!gpu_device_headless_read(buf, bytes, &read_width, &read_height))
    return;
  if (keep > 0 && kept >= keep) {
    static int said;
    if (!said++)
