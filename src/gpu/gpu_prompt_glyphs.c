...
{
    if (g_atlas && g_vertices) return 1;
    if (!gpu_device_ready()) return 0;
    g_atlas = gpu_texture_create(X2_PROMPT_ATLAS_W, X2_PROMPT_ATLAS_H,
                                 GPU_FMT_RGBA8, 1);
    if (g_atlas && !gpu_texture_upload(g_atlas, 0, x2_prompt_atlas,
                                       X2_PROMPT_ATLAS_BYTES)) {
        gpu_texture_destroy(g_atlas);
        g_atlas = 0;
    }
    g_vertices = gpu_buffer_create(
        GPU_BUF_VERTEX,
        X2_PROMPT_QUADS_MAX * 6u * (uint32_t)sizeof(struct PromptVertex));
    if (!g_atlas || !g_vertices) {
        if (g_vertices) gpu_buffer_destroy(g_vertices);
        if (g_atlas) gpu_texture_destroy(g_atlas);
        g_vertices = 0;
        g_atlas = 0;
...
void gpu_prompt_glyphs_frame_begin(void)
{
    ...
    if (ensure_resources()) g_frames_ready++;
}
...
int gpu_prompt_glyphs_render(const float mvp[16])
{
    ...
    for (i = 0; i < count; i++) write_quad(&vertices[i * 6u], &quads[i]);
...
int gpu_prompt_glyphs_selftest(void)
{
    ...
    full.u0 = cell->u0; full.v0 = cell->v0;
    full.u1 = cell->u1; full.v1 = cell->v1;
    half = full;
    half.x0 = 52.0f; half.x1 = 92.0f;
    half.color = 0x80ffffffu;
    write_quad(check, &full);
    if (check[0].v != 1.0f - cell->v1 ||
        check[2].v != 1.0f - cell->v0) {
    ...
    for (i = 0; i < 96u * 64u; i++) {
        ...
        if (p != 0xff0000ffu) changed++;
        if (x < 48u && red > full_red) full_red = red;
        if (x >= 48u && red > half_red) half_red = red;
    }
