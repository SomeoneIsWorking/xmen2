...
SDL_GPUGraphicsPipeline *gpu_pipeline_for(const PipeKey *k) {
  ...
  ci.primitive_type =
      ...
      : k->prim == GPU_PRIM_LINELIST    ? SDL_GPU_PRIMITIVETYPE_LINELIST
                                        : SDL_GPU_PRIMITIVETYPE_TRIANGLELIST;

  ci.rasterizer_state.fill_mode = SDL_GPU_FILLMODE_FILL;
  ci.rasterizer_state.cull_mode = k->cull == GPU_CULL_CW ? SDL_GPU_CULLMODE_BACK
                                  : k->cull == GPU_CULL_CCW
...
SDL_GPUSampler *gpu_sampler_for(int clamp, int mag_point, int min_filter,
  ...
  for (i = 0; i < g_nsamplers; i++)
    if (g_sampler[i].clamp == clamp && g_sampler[i].mag_point == mag_point &&
        g_sampler[i].min_filter == min_filter && g_sampler[i].mip == mip &&
...
void gpu_pipeline_shutdown(void) {
  ...
  for (i = 0; i < g_npipes; i++)
    if (g_pipes[i].pipe)
      SDL_ReleaseGPUGraphicsPipeline(g_gpu, g_pipes[i].pipe);
  g_npipes = 0;
  for (i = 0; i < g_nsamplers; i++)
  ...
  g_nsamplers = 0;
  if (g_vs) {
    SDL_ReleaseGPUShader(g_gpu, g_vs);
    g_vs = NULL;
  }
  if (g_fs) {
