...
static int matrix_inverse(const float in[16], float out[16])
{
    ...
    for (row = 0; row < 4; row++)
        for (column = 0; column < 8; column++)
            rows[row][column] = column < 4 ? in[row * 4 + column]
                                           : (column - 4 == row ? 1.0f : 0.0f);
    for (column = 0; column < 4; column++) {
        pivot = column;
        for (row = column + 1; row < 4; row++)
            if (fabsf(rows[row][column]) > fabsf(rows[pivot][column])) pivot = row;
            ...
            }
        {
            ...
            for (k = 0; k < 8; k++) rows[column][k] /= scale;
        }
        for (row = 0; row < 4; row++) {
            float scale;
            if (row == column) continue;
            scale = rows[row][column];
            for (k = 0; k < 8; k++) rows[row][k] -= scale * rows[column][k];
        }
    }
    for (row = 0; row < 4; row++)
        for (column = 0; column < 4; column++)
...
static int vector_normalize(float v[3])
{
    ...
    if (!isfinite(length) || length < 1e-6f) return 0;
    v[0] /= length;
    v[1] /= length;
    v[2] /= length;
    return 1;
...
static int row_transform(const float in[4], const float matrix[16], float out[4])
{
    int column, k;
    for (column = 0; column < 4; column++) {
        out[column] = 0.0f;
        for (k = 0; k < 4; k++) out[column] += in[k] * matrix[k * 4 + column];
    }
    if (!isfinite(out[3]) || fabsf(out[3]) < 1e-7f) return 0;
    return 1;
}
...
unsigned gpu_shadow_draw_roles(const GpuDraw *draw)
{
    ...
    if (!triangles) return roles;
    roles |= GPU_SHADOW_CASTER;
    emissive = draw->mat_emissive[0] + draw->mat_emissive[1]
               + draw->mat_emissive[2];
...
int gpu_shadow_frame_policy(const GpuDraw *draw, GpuShadowFramePolicy *out)
{
    ...
    if (!draw || !out || !draw->lighting || draw->programmable) return 0;
    ...
    for (z = 0; z <= 1; z++)
        for (y = -1; y <= 1; y += 2)
            for (x = -1; x <= 1; x += 2) {
                ...
                float world[4];
                if (!row_transform(clip, inverse_view_projection, world)) return 0;
    ...
    if (!vector_normalize(out->light_direction)) return 0;
    ...
    if (!vector_normalize(right)) return 0;
    vector_cross(out->light_direction, right, up);
    if (!vector_normalize(up)) return 0;
    ...
    for (i = 0; i < 8; i++) {
        ...
        for (axis = 0; axis < 3; axis++) {
            if (projected[axis] < minv[axis]) minv[axis] = projected[axis];
            if (projected[axis] > maxv[axis]) maxv[axis] = projected[axis];
        }
    }
