...
static int valid_resolution(uint32_t resolution) {
  return resolution == 512 || resolution == 1024 || resolution == 2048 ||
         resolution == 4096;
}
...
static SDL_GPUGraphicsPipeline *pipeline_for(const GpuDraw *draw) {
  ...
  info.rasterizer_state.cull_mode =
      draw->cull == GPU_CULL_CW    ? SDL_GPU_CULLMODE_BACK
      : draw->cull == GPU_CULL_CCW ? SDL_GPU_CULLMODE_FRONT
...
void gpu_shadow_shutdown(void) {
  ...
  g_pipeline_count = 0;
  if (g_sampler)
    SDL_ReleaseGPUSampler(g_gpu, g_sampler);
  if (g_texture)
    SDL_ReleaseGPUTexture(g_gpu, g_texture);
  if (g_vertex_shader)
