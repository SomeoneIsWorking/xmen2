                         ...
                         GpuDraw *receiver, uint32_t *pixels)
{
    ...
    if (!gpu_offscreen_begin(SHADOW_TEST_W, SHADOW_TEST_H,
...
{
    return (pixel & 0xffu) + ((pixel >> 8) & 0xffu) +
           ((pixel >> 16) & 0xffu);
}
#endif
...
int gpu_shadow_selftest(void)
{
    ...
    if (!caster_buffer || !receiver_buffer ||
        ...
        !gpu_buffer_upload(receiver_buffer, 0, receiver_vertices,
                           sizeof receiver_vertices)) goto done;
    ...
    if (!render_answer(0, 1, &caster, &receiver, disabled) ||
        ...
        !render_answer(1, 0, &caster, &receiver, no_caster)) goto done;
    ...
    for (unsigned i = 0; i < SHADOW_TEST_W * SHADOW_TEST_H; i++) {
        ...
        if (on + 60 < off) darker++;
        if (on + 60 < open) caster_dependent++;
        if (on == off) unchanged++;
    }
    ...
    if (caster_buffer) gpu_buffer_destroy(caster_buffer);
    if (receiver_buffer) gpu_buffer_destroy(receiver_buffer);
    gpu_device_destroy();
    return result;
#endif
}
