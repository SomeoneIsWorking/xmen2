...
static int reserve_frames(size_t required)
{
    ...
    for (i = 0; i < g_movie_audio.queued_frames; ++i) {
        size_t old = (g_movie_audio.read_frame + i)
...
int movie_audio_queue(const float *stereo, size_t frames)
{
    ...
    int ok = 0;
    pthread_mutex_lock(&g_movie_audio_lock);
    if (!g_movie_audio.active || (!stereo && frames)) goto done;
    if (!reserve_frames(g_movie_audio.queued_frames + frames)) goto done;
    for (i = 0; i < frames; ++i) {
        size_t dst = (g_movie_audio.read_frame
                    + g_movie_audio.queued_frames + i)
                   % g_movie_audio.capacity_frames;
...
{
    pthread_mutex_lock(&g_movie_audio_lock);
    if (g_movie_audio.active) g_movie_audio.paused = 0;
    pthread_mutex_unlock(&g_movie_audio_lock);
}
...
{
    pthread_mutex_lock(&g_movie_audio_lock);
    if (g_movie_audio.active) g_movie_audio.paused = paused ? 1 : 0;
    pthread_mutex_unlock(&g_movie_audio_lock);
}
...
{
    size_t index = (g_movie_audio.read_frame + frame)
                 % g_movie_audio.capacity_frames;
...
void movie_audio_mix(float *mix, int frames, int output_rate)
{
    ...
    for (i = 0; i < frames; ++i) {
        ...
        if (g_movie_audio.source_fraction >= 1.0) {
            ...
            g_movie_audio.read_frame = (g_movie_audio.read_frame + consumed)
                                     % g_movie_audio.capacity_frames;
...
double movie_audio_queued_seconds(void)
{
    double seconds;
    pthread_mutex_lock(&g_movie_audio_lock);
    seconds = g_movie_audio.sample_rate <= 0 ? 0.0
...
void movie_audio_report(void)
{
    ...
    printf("          movie audio: %lu open(s), %llu frame(s) queued, "
           ...
           g_movie_audio.queued_total, g_movie_audio.underruns,
