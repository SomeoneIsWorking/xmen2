...
int x2_settings_assign_keyboard(X2Settings *settings, unsigned profile,
  ...
  if (player > 0)
    clear_player_controllers(&changed, (unsigned)player);
...
int x2_settings_assign_controller(X2Settings *settings, const char *id,
  ...
  if (player == X2_SETTINGS_UNASSIGNED) {
    if (slot >= 0) {
      memset(&changed.controller[slot], 0, sizeof changed.controller[slot]);
  ...
  if (player > 0)
    clear_player_keyboards(&changed, (unsigned)player);
  if (!x2_settings_input_assignments_valid(&changed))
    return 0;
  *settings = changed;
  return 1;
