...
void x86_watch_trace_reset(void) { note_count = 0; }
...
void x86_watch_trace_dump(FILE *out) {
  static const char *labels[] = {"ENTER guest", "CALL host  ", "host RET   ",
                                 "guest RET  "};
  ...
  for (i = note_count - count; i < note_count; i++) {
    ...
    fprintf(out, "[TRACE]   tid %-5lu %s  addr=0x%08x esp=0x%08x\n",
            note->thread_id, labels[note->kind & 3], note->address, note->esp);
  }
}
