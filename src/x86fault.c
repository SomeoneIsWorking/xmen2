...
static void dump_regs(FILE *o, const CONTEXT *c) {
  ...
  fprintf(o, "[FAULT]   eax=%08lx ecx=%08lx edx=%08lx ebx=%08lx\n",
          (unsigned long)c->Eax, (unsigned long)c->Ecx, (unsigned long)c->Edx,
          (unsigned long)c->Ebx);
...
static int call_site_before(uint32_t v, uint32_t *site) {
  ...
  for (i = 0; i < sizeof len / sizeof len[0]; i++) {
    ...
    if (!mem_readable(p, (size_t)len[i]))
      continue;
    if (len[i] == 5 && p[0] == 0xE8) {
      *site = s;
      return 1;
...
static void dump_stack(FILE *o, uint32_t esp) {
  ...
  if (!mem_readable((void *)(uintptr_t)esp, 4)) {
    fprintf(o,
            "[FAULT]   stack at esp: UNREADABLE (esp=%08x) -- the stack "
  ...
  for (i = 0; i < f_stack_slots; i++) {
    ...
    guest_note(v, g, sizeof g);
    {
      ...
      if (call_site_before(v, &site)) {
        ...
        fprintf(o,
                "[FAULT]     %08x: %08x  %s%s  <- RET from call at "
                "%08x %s%s\n",
...
static int fatal_code(DWORD c) {
  return c == EXCEPTION_ACCESS_VIOLATION ||
         ...
         c == EXCEPTION_FLT_INVALID_OPERATION || c == EXCEPTION_FLT_OVERFLOW ||
         ...
         || c == EXCEPTION_BREAKPOINT || c == EXCEPTION_ILLEGAL_INSTRUCTION;
}
...
static LONG CALLBACK fault_veh(EXCEPTION_POINTERS *ep) {
  ...
  if (!fatal_code(er->ExceptionCode)) {
    ...
    if (i < 16) {
      ...
      fprintf(o,
              "[FAULT] first non-fatal exception code=0x%08lx at %p "
  ...
  if (InterlockedIncrement(&f_seen) != 1)
    return EXCEPTION_CONTINUE_SEARCH;
...
static void fault_report(void) {
  ...
  if (f_seen)
    fprintf(o,
            "[FAULT] %ld fatal exception(s) this run (reported the first)"
            "; %ld non-fatal first-chance exception(s) passed through\n",
            (long)f_seen, (long)f_other);
  else
    fprintf(o,
            "[FAULT] no fatal exception reached the handler this run "
...
void x86_fault_install(void) {
  ...
  if (!f_veh) {
    fprintf(o,
            "[FAULT] AddVectoredExceptionHandler FAILED (%lu) -- there "
