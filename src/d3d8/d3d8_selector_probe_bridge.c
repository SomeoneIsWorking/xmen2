...
static D3D8SelectorMultiplyStep g_world_matrix_multiply_chain[
...
static MatrixMultiplyRecord g_matrix_multiply_history[
...
static TransformSetRecord g_transform_set_history[
    MATRIX_SET_HISTORY_CAPACITY];
...
void d3d8_selector_probe_title_builder_enter(CPU *C)
{
    ...
    if (!d3d8_selector_probe_enabled()) return;
...
{
    TransformSetRecord *record = &g_transform_set_history[
...
{
    MatrixCopyRecord *record = &g_matrix_copy_history[
        g_matrix_copy_count % MATRIX_SET_HISTORY_CAPACITY];

    record->destination = RD32(C->esp + 4u);
    record->caller = RD32(C->esp);
    record->source = RD32(C->esp + 8u);
    record->source_readable = guest_memory_is_readable(
        record->source, sizeof record->source_value);
    if (record->source_readable)
        memcpy(record->source_value,
               guest_memory_const_pointer(record->source),
...
{
    MatrixMultiplyRecord *record = &g_matrix_multiply_history[
    ...
    record->inputs_readable =
        ...
        && guest_memory_is_readable(record->right,
...
static void find_matrix_set(uint32_t matrix_guest)
{
    uint32_t available = g_matrix_set_count < MATRIX_SET_HISTORY_CAPACITY
        ? g_matrix_set_count : MATRIX_SET_HISTORY_CAPACITY;
    uint32_t offset;
    ...
    for (offset = 0; offset < available; ++offset) {
        ...
        if (record->attr + 12u != matrix_guest) continue;
...
static void find_matrix_multiply(uint32_t matrix_guest)
{
    ...
    for (offset = 0; offset < available; ++offset) {
        ...
        const MatrixMultiplyRecord *record = &g_matrix_multiply_history[
            sequence % MATRIX_SET_HISTORY_CAPACITY];
        if (record->output != matrix_guest) continue;
    ...
    while (g_world_matrix_multiply_chain_count
        ...
        if (!found) return;
        {
            ...
            memcpy(step->left_value, found->left_value,
                   sizeof step->left_value);
            memcpy(step->right_value, found->right_value,
                   sizeof step->right_value);
            {
                uint32_t copy_available =
                    g_matrix_copy_count < MATRIX_SET_HISTORY_CAPACITY
                        ? g_matrix_copy_count : MATRIX_SET_HISTORY_CAPACITY;
                uint32_t copy_offset;
                for (copy_offset = 0; copy_offset < copy_available;
                     ++copy_offset) {
                    uint32_t sequence =
                        g_matrix_copy_count - 1u - copy_offset;
                    const MatrixCopyRecord *copy = &g_matrix_copy_history[
                        sequence % MATRIX_SET_HISTORY_CAPACITY];
                    if (copy->destination != found->left) continue;
                    ...
                    memcpy(step->left_copy_source_value,
                           copy->source_value,
            ...
            }
            {
                uint32_t set_available =
                    g_transform_set_count < MATRIX_SET_HISTORY_CAPACITY
                        ...
                        : MATRIX_SET_HISTORY_CAPACITY;
                uint32_t set_offset;
                for (set_offset = 0; set_offset < set_available;
                     ++set_offset) {
                    uint32_t sequence =
                        g_transform_set_count - 1u - set_offset;
                    const TransformSetRecord *set =
                        &g_transform_set_history[
                            sequence % MATRIX_SET_HISTORY_CAPACITY];
                    if (set->destination != found->left) continue;
                    ...
                    step->title_builder_translation =
                        set->title_builder_translation;
                    step->title_builder_rotation = set->title_builder_rotation;
                    memcpy(step->title_builder_scale,
                           set->title_builder_scale,
...
static void selector_probe_register_matrix_source(void)
{
    if (!d3d8_selector_probe_enabled()) return;
    x86_register_override("libIGGfx.dll", 0x1003e9e0u,
                          selector_probe_set_matrix);
    ...
    unsigned long frame, D3D8SelectorProbeTicket *ticket)
{
    ...
    memcpy(evidence.world_matrix_multiply_chain,
           g_world_matrix_multiply_chain,
