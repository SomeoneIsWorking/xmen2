...
D3D8State *d3d8_device_state(void) { return &g_dev.state; }
...
static void dev_GetDeviceCaps(D3D8Object *self, CPU *C) {
  D3DCAPS8 *caps = (D3DCAPS8 *)d3d8_guest_ptr(d3d8_arg(C, 0), "caps block");
  (void)self;
  if (!caps) {
...
static void dev_GetDisplayMode(D3D8Object *self, CPU *C) {
  D3DDISPLAYMODE *m = (D3DDISPLAYMODE *)d3d8_guest_ptr(d3d8_arg(C, 0), "mode");
  (void)self;
  if (!m) {
    d3d8_ret(C, D3DERR_INVALIDCALL);
    return;
...
static void dev_GetCreationParameters(D3D8Object *self, CPU *C) {
  D3DDEVICE_CREATION_PARAMETERS *p =
      (D3DDEVICE_CREATION_PARAMETERS *)d3d8_guest_ptr(d3d8_arg(C, 0),
                                                      "creation parameters");
  (void)self;
  if (!p) {
    d3d8_ret(C, D3DERR_INVALIDCALL);
...
static void dev_Clear(D3D8Object *self, CPU *C) {
  ...
  gpu_frame_clear(mask, (float)((colour >> 16) & 0xFF) / 255.0f,
                  (float)((colour >> 8) & 0xFF) / 255.0f,
                  (float)((colour) & 0xFF) / 255.0f,
                  (float)((colour >> 24) & 0xFF) / 255.0f, depth, stencil);
  g_dev.clears++;
  d3d8_ret(C, D3D_OK);
}
...
static void dev_GetRenderState(D3D8Object *self, CPU *C) {
  ...
  (void)self;
  out = (uint32_t *)d3d8_guest_ptr(d3d8_arg(C, 1), "value");
  if (!out) {
    d3d8_ret(C, D3DERR_INVALIDCALL);
    return;
  }
...
static void dev_GetTextureStageState(D3D8Object *self, CPU *C) {
  ...
  (void)self;
  out = (uint32_t *)d3d8_guest_ptr(d3d8_arg(C, 2), "value");
  if (!out) {
    d3d8_ret(C, D3DERR_INVALIDCALL);
    return;
  }
...
static void dev_SetTransform(D3D8Object *self, CPU *C) {
  ...
  const float *m = (const float *)d3d8_guest_ptr(d3d8_arg(C, 1), "matrix");
  (void)self;
  if (!m) {
    d3d8_ret(C, D3DERR_INVALIDCALL);
    return;
  }
  if (which >= D3D8_MAX_TRANSFORMS) {
    d3d8_ret(C, D3DERR_INVALIDCALL);
    return;
  }
...
static void dev_GetTransform(D3D8Object *self, CPU *C) {
  uint32_t which = d3d8_arg(C, 0);
  float *m = (float *)d3d8_guest_ptr(d3d8_arg(C, 1), "matrix");
  (void)self;
...
static void dev_SetViewport(D3D8Object *self, CPU *C) {
  const D3DVIEWPORT8 *v =
      (const D3DVIEWPORT8 *)d3d8_guest_ptr(d3d8_arg(C, 0), "viewport");
  (void)self;
  if (!v) {
    d3d8_ret(C, D3DERR_INVALIDCALL);
    return;
...
static void dev_GetViewport(D3D8Object *self, CPU *C) {
  D3DVIEWPORT8 *v = (D3DVIEWPORT8 *)d3d8_guest_ptr(d3d8_arg(C, 0), "viewport");
  (void)self;
  if (!v) {
    d3d8_ret(C, D3DERR_INVALIDCALL);
    return;
...
static void dev_ValidateDevice(D3D8Object *self, CPU *C) {
  uint32_t *passes = (uint32_t *)d3d8_guest_ptr(d3d8_arg(C, 0), "pass count");
  (void)self;
...
void d3d8_vertex_shader_binding_line(char *buf, size_t n) {
  ...
  for (i = 0; i < g_vs_nseen && at + 20 < n; i++)
    at += (size_t)snprintf(buf + at, n - at, " 0x%x%s", g_vs_seen[i].handle,
...
static void dev_GetVertexShader(D3D8Object *self, CPU *C) {
  uint32_t *out = (uint32_t *)d3d8_guest_ptr(d3d8_arg(C, 0), "handle");
  (void)self;
  if (!out) {
    d3d8_ret(C, D3DERR_INVALIDCALL);
    return;
...
static void dev_CreateVertexShader(D3D8Object *self, CPU *C) {
  const uint32_t *decl =
      (const uint32_t *)d3d8_guest_ptr(d3d8_arg(C, 0), "vertex declaration");
  const uint32_t *code = (const uint32_t *)d3d8_guest_ptr(
      d3d8_arg(C, 1), "vertex shader bytecode");
  uint32_t *out = (uint32_t *)d3d8_guest_ptr(d3d8_arg(C, 2), "shader handle");
  uint32_t handle;
  (void)self;
  if (!decl || !out) {
    d3d8_ret(C, D3DERR_INVALIDCALL);
...
static void dev_DeleteVertexShader(D3D8Object *self, CPU *C) {
  ...
  (void)self;
  if (g_dev.state.vertex_shader == h)
    g_dev.state.vertex_shader = 0;
...
static void vsc_walk(CPU *C, uint32_t *out, int *nout) {
  ...
  for (sp = C->esp + 4u; sp < C->esp + 4u + 512u && n < VSC_DEPTH; sp += 4u) {
    ...
    if (!guest_memory_is_readable(sp & ~4095u, 4096u))
      break;
    w = RD32(sp);
    if (!x86_module_for(w))
      continue;
...
static void vsc_note(uint32_t ret, uint32_t first, uint32_t count) {
  ...
  for (i = 0; i < g_nvsc; i++)
    if (g_vsc[i].ret == ret) {
      ...
      if (first < g_vsc[i].lo)
        g_vsc[i].lo = first;
      if (first + count > g_vsc[i].hi)
        g_vsc[i].hi = first + count;
      return;
    }
  if (g_nvsc == VSC_SITES) {
    g_vsc_overflow++;
...
static void dev_SetVertexShaderConstant(D3D8Object *self, CPU *C) {
  ...
  const float *data =
      (const float *)d3d8_guest_ptr(d3d8_arg(C, 1), "constants");
  (void)self;
...
static void dev_GetVertexShaderConstant(D3D8Object *self, CPU *C) {
  uint32_t first = d3d8_arg(C, 0), count = d3d8_arg(C, 2);
  float *data = (float *)d3d8_guest_ptr(d3d8_arg(C, 1), "constants");
  (void)self;
  if (!data || first >= D3D8_MAX_VS_CONSTANTS ||
      count > D3D8_MAX_VS_CONSTANTS - first) {
    d3d8_ret(C, D3DERR_INVALIDCALL);
...
static void dev_SetGammaRamp(D3D8Object *self, CPU *C) {
  const uint16_t *ramp =
      (const uint16_t *)d3d8_guest_ptr(d3d8_arg(C, 1), "gamma ramp");
  (void)self;
  if (!ramp) {
    d3d8_ret(C, 0);
    return;
  } /* SetGammaRamp returns void */
...
static void dev_GetGammaRamp(D3D8Object *self, CPU *C) {
  uint16_t *out = (uint16_t *)d3d8_guest_ptr(d3d8_arg(C, 0), "gamma ramp");
  (void)self;
  if (!out) {
    d3d8_ret(C, 0);
    return;
...
static int fill_request(D3D8DrawRequest *req, uint32_t prim, uint32_t count,
  ...
  D3D8Object *vb = g_dev.state.stream[0].guest_ptr
                       ...
                       : NULL;
  D3D8Object *ib =
      g_dev.state.indices ? d3d8_object_from_guest(g_dev.state.indices) : NULL;
  ...
  if (tx0)
    (void)d3d8_resource_texture_provenance(tx0, &req->texture_provenance);
...
static void up_vertices_destroy(void) {
  if (g_up_vertices)
    gpu_buffer_destroy(g_up_vertices);
  g_up_vertices = 0;
  g_up_capacity = 0;
}
...
static void dev_DrawPrimitiveUP(D3D8Object *self, CPU *C) {
  ...
  const void *data = d3d8_guest_ptr(data_guest, "inline vertex data");
