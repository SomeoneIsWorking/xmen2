...
static void dev_SetRenderState(D3D8Object *self, CPU *C)
{
    ...
    if (which == D3D8_RS_LIGHTING)
        d3d8_lightlog("SETRENDERSTATE t=%lu LIGHTING=%lu",
...
{
    const uint32_t *decl = (const uint32_t *)d3d8_guest_ptr(d3d8_arg(C, 0),
                                                       "vertex declaration");
    const uint32_t *code = (const uint32_t *)d3d8_guest_ptr(d3d8_arg(C, 1),
...
static void dev_SetVertexShaderConstant(D3D8Object *self, CPU *C)
{
    ...
    const float *data = (const float *)d3d8_guest_ptr(d3d8_arg(C, 1), "constants");
    (void)self;
