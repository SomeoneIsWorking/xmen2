...
int d3d8_fvf_layout(uint32_t fvf, D3D8VertexLayout *out)
{
    ...
    if (fvf & D3DFVF_SPECULAR) { out->specular_offset = (int)off; off += 4; }
    if (ntex) {
...
static struct { uint32_t fvf; unsigned long n; } g_seen[FVF_SEEN_MAX];
static int g_seen_n;
...
void d3d8_fvf_note(uint32_t fvf)
{
    ...
    for (i = 0; i < g_seen_n; i++)
        if (g_seen[i].fvf == fvf) { g_seen[i].n++; return; }
    if (g_seen_n == FVF_SEEN_MAX) { g_dropped++; return; }
