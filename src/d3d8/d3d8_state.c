...
int d3d8_state_set_render(D3D8State *s, uint32_t which, uint32_t value) {
  if (!range(which, D3D8_MAX_RENDER_STATES, "render state"))
    return 0;
...
int d3d8_state_set_stage(D3D8State *s, uint32_t stage, uint32_t which,
                         uint32_t value) {
  if (!range(stage, D3D8_MAX_STAGES, "texture stage"))
    return 0;
  if (!range(which, D3D8_MAX_STAGE_STATES, "texture stage state"))
    return 0;
...
void d3d8_state_report(const D3D8State *s) {
  ...
  for (i = 0; i < D3D8_MAX_RENDER_STATES; i++)
    nrender += s->render[i].set != 0;
  for (i = 0; i < D3D8_MAX_STAGES; i++)
    for (k = 0; k < D3D8_MAX_STAGE_STATES; k++)
      nstage += s->stage[i][k].set != 0;
  for (i = 0; i < D3D8_MAX_TRANSFORMS; i++)
    ntransform += s->transform_set[i] != 0;
   ...
   */
  {
    ...
    for (j = 0; j < D3D8_MAX_RENDER_STATES; j++) {
      if (!s->render[j].set)
        continue;
      if (d3d8_drawcall_reads_state((uint32_t)j))
        continue;
      unread++;
      if (!any++)
