...
D3D8Object *d3d8_vertexbuffer_new(uint32_t bytes, uint32_t usage, uint32_t fvf,
                                  uint32_t pool) {
  return buffer_new(D3D8_IF_IDirect3DVertexBuffer8, bytes, fvf, 0, usage, pool);
}
...
static void tex_GetType(D3D8Object *self, CPU *C) {
  (void)self;
  d3d8_ret(C, D3DRTYPE_TEXTURE);
}
static void tex_GetLevelCount(D3D8Object *self, CPU *C) {
  d3d8_ret(C, res_of(self)->levels);
}
static void tex_SetLOD(D3D8Object *self, CPU *C) {
  (void)self;
  d3d8_ret(C, 0);
}
static void tex_GetLOD(D3D8Object *self, CPU *C) {
  (void)self;
  d3d8_ret(C, 0);
}
...
static void lock_sub(D3D8Object *self, CPU *C, uint32_t face, uint32_t level,
  ...
  if (rect) {
    ...
    if (bpp) {
      ...
      lr[1] +=
          (top / 4u) * lr[0] + (left / 4u) * (lr[0] / (blocks ? blocks : 1u));
    }
  }
  r->locked = 1;
  r->locked_sub = sub;
...
static void unlock_sub(D3D8Object *self, CPU *C, uint32_t face,
                       uint32_t level) {
  Resource *r = res_of(self);
...
static void tex_AddDirtyRect(D3D8Object *self, CPU *C) {
  (void)self;
  d3d8_ret(C, D3D_OK);
} /* nothing is deferred, so nothing dirties */
...
static void cube_GetType(D3D8Object *self, CPU *C) {
  (void)self;
  d3d8_ret(C, D3DRTYPE_CUBETEXTURE);
}
...
static const D3D8MethodFn g_tex_impl[] = {res_QueryInterface,
                                          res_AddRef,
                                          res_Release,
                                          res_GetDevice,
                                          NULL,
                                          NULL,
                                          NULL, /* 4-6 private data */
                                          res_SetPriority,
                                          res_GetPriority,
                                          res_PreLoad,
                                          tex_GetType,
...
static const D3D8MethodFn g_cube_impl[] = {res_QueryInterface,
                                           res_AddRef,
                                           res_Release,
                                           res_GetDevice,
                                           NULL,
...
static void buf_Lock(D3D8Object *self, CPU *C) {
  ...
  }
  {
    ...
    uint32_t known = D3DLOCK_READONLY | D3DLOCK_NOOVERWRITE | D3DLOCK_DISCARD;
    g_lock_calls++;
...
void d3d8_buffer_lock_counts(unsigned long *locks, unsigned long *discard,
  ...
  *locks = g_lock_calls;
  *discard = g_lock_discard;
  *nooverwrite = g_lock_nooverwrite;
  *unlocks = g_unlocks;
  *bytes = g_unlock_bytes;
...
static void vbuf_GetType(D3D8Object *self, CPU *C) {
  (void)self;
  d3d8_ret(C, D3DRTYPE_VERTEXBUFFER);
}
static void ibuf_GetType(D3D8Object *self, CPU *C) {
  (void)self;
  d3d8_ret(C, D3DRTYPE_INDEXBUFFER);
}
...
static const D3D8MethodFn g_vbuf_impl[] = {res_QueryInterface,
                                           res_AddRef,
                                           res_Release,
                                           res_GetDevice,
                                           NULL,
...
static const D3D8MethodFn g_ibuf_impl[] = {res_QueryInterface,
                                           res_AddRef,
                                           res_Release,
                                           res_GetDevice,
                                           NULL,
