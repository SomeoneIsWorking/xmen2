...
static void resize_surface(D3D8Surface *surface, uint32_t width,
                           uint32_t height) {
  if (!surface)
    return;
  surface->width = width;
...
void d3d8_live_resolution_unbind(void) { g_active = (ActivePresentation){0}; }
...
int d3d8_live_resolution_apply(uint32_t width, uint32_t height, char *why,
  ...
  if (backbuffer->kind != D3D8_SURF_BACKBUFFER ||
      ...
      (depth &&
       (depth->kind != D3D8_SURF_DEPTHSTENCIL || !depth->bytes_per_pixel)))
