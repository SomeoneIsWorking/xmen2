    ...
    FILE *output, const D3D8SelectorDrawEvidence *evidence)
{
    ...
    for (step_index = 0;
         step_index < evidence->world_matrix_multiply_chain_count;
         ++step_index) {
        ...
        fprintf(output,
                ...
                "\"left_value\":[",
                step_index ? "," : "", step->output, step->caller,
                step->left, step->right,
                step->inputs_readable ? "true" : "false",
                step->left_copy_found ? "true" : "false",
                step->left_copy_caller, step->left_copy_source,
                step->left_copy_source_readable ? "true" : "false",
                step->left_transform_set_found ? "true" : "false",
                step->left_transform_set_caller,
                step->left_transform_set_source,
