...
void d3d8_texture_luma_note(uint32_t handle, uint32_t format,
                            ...
                            const uint8_t *pixels, uint32_t bytes)
{
    ...
    case D3DFMT_DXT1:
        for (offset = 0; offset + 8 <= bytes; offset += 8) {
            sum += (luma_565((uint16_t)(pixels[offset]
                                       | (pixels[offset + 1] << 8)))
                  + luma_565((uint16_t)(pixels[offset + 2]
                                       | (pixels[offset + 3] << 8)))) * 0.5;
            samples++;
        }
        break;
    case D3DFMT_DXT2:
    ...
    case D3DFMT_DXT5:
        for (offset = 0; offset + 16 <= bytes; offset += 16) {
            sum += (luma_565((uint16_t)(pixels[offset + 8]
                                       | (pixels[offset + 9] << 8)))
                  + luma_565((uint16_t)(pixels[offset + 10]
                                       | (pixels[offset + 11] << 8)))) * 0.5;
            samples++;
        }
        break;
    default:
    ...
    for (index = 0; index < g_luma_count; ++index) {
        if (g_luma[index].handle != handle) continue;
...
void d3d8_texture_luma_report(void)
{
    ...
    printf("        texture brightness: %d texture(s) measured%s, %lu upload(s) "
           ...
           g_luma_dropped ? " (TABLE FULL -- more exist)" : "",
           g_luma_unreadable,
           g_luma_count ? total / g_luma_count : 0.0, dark);
    if (!g_luma_count) {
    ...
    for (rank = 0; rank < 10 && rank < g_luma_count; ++rank) {
        int darkest = -1;
        for (index = 0; index < g_luma_count; ++index) {
            if (!g_luma[index].width) continue;
