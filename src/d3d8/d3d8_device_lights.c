...
void d3d8_dev_SetMaterial(D3D8Object *self, CPU *C)
{
    ...
    d3d8_lightlog("SETMATERIAL t=%lu diffuse=%.4f,%.4f,%.4f,%.4f "
             ...
             "specular=%.4f,%.4f,%.4f,%.4f power=%.2f", d3d8_lightlog_ms(),
             m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7],
             m[12], m[13], m[14], m[15], m[8], m[9], m[10], m[11], m[16]);
    memcpy(d3d8_device_state()->material, m, sizeof d3d8_device_state()->material);
...
void d3d8_dev_SetLight(D3D8Object *self, CPU *C)
{
    ...
    if (!l)
        d3d8_lightlog("SETLIGHT t=%lu idx=%lu NULL-POINTER",
    ...
    memcpy(d3d8_device_state()->light[idx], l, sizeof d3d8_device_state()->light[0]);
