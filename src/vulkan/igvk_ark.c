...
void igvk_vtable_set(uint32_t vtable, int slot, void (*fn)(CPU *),
  ...
  WR32(vtable + (uint32_t)slot * 4u, x86_native_callback(fn, owner, name, ctx));
}
  ...
  for (k = 0; k < nslots; k++)
    if (RD32(vtable + (uint32_t)k * 4u) == 0) {
      ...
      WR32(vtable + (uint32_t)k * 4u,
           x86_native_callback(unimplemented, owner, "<unimplemented slot>",
                               &recs[k]));
      n++;
    }
