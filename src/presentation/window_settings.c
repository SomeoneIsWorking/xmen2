                              ...
                              unsigned height)
{
    ...
    if (!modes) return 0;
    for (i = 0; i < count; i++)
        if ((unsigned)modes[i]->w == width &&
            (unsigned)modes[i]->h == height &&
...
{
    if (!window || !settings) {
        if (why && whyn > 0) snprintf(why, (size_t)whyn,
                                     "window and settings are required");
        return 0;
    }
    if (settings->window_mode == X2_WINDOW_WINDOWED) {
        ...
        if (!SDL_SetWindowSize(window, (int)settings->width,
                              (int)settings->height))
        ...
        if (!set_exclusive_mode(window, settings->width, settings->height)) {
            if (why && whyn > 0)
                snprintf(why, (size_t)whyn,
                         "display has no %ux%u exclusive mode",
        ...
        if (why && whyn > 0) snprintf(why, (size_t)whyn,
                                     "unknown window mode %d",
                                     (int)settings->window_mode);
        return 0;
    }
    if (!SDL_SyncWindow(window)) return fail(why, whyn, "synchronising window");
    g_owns_geometry = 1;
    if (why && whyn > 0)
