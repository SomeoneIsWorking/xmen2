...
{
    if (!g_blackout.armed) return 0;
    if (g_blackout.frames_blacked >= BLACKOUT_MAX_FRAMES ||
        guest_clock_elapsed_s() - g_blackout.armed_at_s >=
            BLACKOUT_TIMEOUT_S) {
...
size_t x2_boot_blackout_report(char *out, size_t size)
{
    ...
    wrote = snprintf(out, size,
                     ...
                     "black%s%s\n",
                     g_blackout.armed ? "ARMED" : "closed",
                     g_blackout.mode, g_blackout.frames_blacked,
                     g_blackout.ended[0] ? "; " : "",
...
{
    if (g_blackout.armed) g_blackout.frames_blacked++;
}
