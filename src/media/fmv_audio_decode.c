...
static int queue_output(X2FmvAudioDecode *decode, uint8_t *output, int frames)
{
    if (frames > 0 && decode->sink.queue_stereo_f32
            && !decode->sink.queue_stereo_f32(decode->sink.userdata,
                (const float *)output, (size_t)frames, decode->sample_rate))
        return AVERROR(ENOMEM);
    if (frames > 0) decode->decoded_samples += (unsigned long)frames;
    return frames;
}
...
static int convert_frame(X2FmvAudioDecode *decode)
{
    ...
    uint8_t *output = NULL;
    int result;
    if (av_samples_alloc(&output, NULL, 2, capacity,
    ...
    result = swr_convert(decode->resampler, &output, capacity,
                         ...
                         decode->frame->nb_samples);
    av_frame_unref(decode->frame);
    if (result >= 0) result = queue_output(decode, output, result);
    av_freep(&output);
    return result;
}
...
static X2FmvDrainResult receive_step(void *userdata)
{
    ...
    if (result >= 0) result = convert_frame(decode);
    if (result < 0) {
        decode->error = result;
...
static int receive_all(X2FmvAudioDecode *decode)
{
    ...
    for (;;) {
        ...
        if (result == X2_FMV_DRAIN_FAILED) return decode->error;
        return received;
    }
}
...
{
    X2FmvAudioDecode *decode = (X2FmvAudioDecode *)userdata;
    int64_t delay = swr_get_delay(decode->resampler,
                                  decode->codec->sample_rate);
    int capacity;
    int result;
    uint8_t *output = NULL;
    if (delay <= 0) return X2_FMV_DRAIN_COMPLETE;
    capacity = (int)av_rescale_rnd(delay, decode->sample_rate,
                                  decode->codec->sample_rate, AV_ROUND_UP);
    if (capacity <= 0) return X2_FMV_DRAIN_COMPLETE;
    if (av_samples_alloc(&output, NULL, 2, capacity,
                         AV_SAMPLE_FMT_FLT, 0) < 0) {
...
static const X2FmvDecoderDrainOps g_drain_ops = {
    send_flush, receive_step, flush_tail
};
                                             ...
                                             int *error)
{
    ...
    if (!decode) {
        ...
        if (error) *error = AVERROR(ENOMEM);
        return NULL;
    }
    decode->codec = codec;
    decode->sample_rate = codec->sample_rate;
    if (sink) decode->sink = *sink;
    result = swr_alloc_set_opts2(&decode->resampler, &stereo,
        AV_SAMPLE_FMT_FLT, decode->sample_rate, &codec->ch_layout,
        codec->sample_fmt, codec->sample_rate, 0, NULL);
    if (result >= 0) result = swr_init(decode->resampler);
                                    ...
                                    const AVPacket *packet)
{
    ...
    if (result == AVERROR(EAGAIN)) {
        ...
        if (result >= 0) result = avcodec_send_packet(decode->codec, packet);
    }
