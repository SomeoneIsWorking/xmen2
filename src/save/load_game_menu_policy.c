...
static size_t visible_count(size_t total, size_t first)
{
    ...
    if (first >= total) return 0u;
    remaining = total - first;
    return remaining < X2_LOAD_GAME_VISIBLE_ROWS
        ? remaining : X2_LOAD_GAME_VISIBLE_ROWS;
}
                            ...
                            X2LoadGameMenuPlan *out)
{
    ...
    for (slot = 0u; slot < X2_LOAD_GAME_MANUAL_SLOTS; slot++) {
        ...
        if ((manual_present_mask & (uint16_t)(1u << slot)) == 0u) continue;
                                  ...
                                  X2LoadGameMenuWindow *window, int delta)
{
    ...
    if (delta > 0) {
        ...
        selected = amount >= plan->count - window->selected
            ? plan->count - 1u : window->selected + amount;
    } else {
        size_t amount = (size_t)(-(delta + 1)) + 1u;
        selected = amount > window->selected
            ? 0u : window->selected - amount;
    }
    if (selected == window->selected) return 0;
    window->selected = selected;
    if (selected < window->first) window->first = selected;
                                    ...
                                    X2LoadGameMenuWindow *window, size_t row)
{
    ...
    if (logical == window->selected) return 0;
