...
static const char *const POINT_NAME[SAVE_TRACE_POINT_COUNT] = {
    ...
    "load_0049f140", "save_004aeb80", "save_004b15b0",
    "save_004b1746", "save_004b177a", "save_mode_cycle",
    "map_00484ce0",  "lock_combat",   "extraction_save_command"};
...
static int valid_answer(SaveTraceAnswer answer) {
  return answer >= SAVE_TRACE_ANSWER_UNKNOWN && answer <= SAVE_TRACE_ANSWER_YES;
}
...
static void write_report(ReportWriter *writer, const char *format, ...) {
  ...
  count =
      vsnprintf(writer->out ? writer->out + writer->used : NULL,
                writer->out ? writer->size - writer->used : 0, format, args);
  va_end(args);
  if (count > 0)
...
SaveTraceResult save_trace_load_manager(SaveTrace *trace,
                                        ...
                                        uint32_t buffer) {
  return record(trace, SAVE_TRACE_LOAD_004AED10, metadata_branch, NULL, mode,
                state, device, selection, buffer);
}

SaveTraceResult save_trace_mode_cycle(SaveTrace *trace, uint32_t previous_mode,
                                      uint32_t next_mode) {
  return record(trace, SAVE_TRACE_SAVE_MODE_CYCLE, SAVE_TRACE_ANSWER_UNKNOWN,
                NULL, previous_mode, next_mode, 0, 0, 0);
}
...
int save_trace_event_at(const SaveTrace *trace, size_t chronological_index,
  ...
  if (!trace || !out || chronological_index >= trace->retained)
    return 0;
...
static void render_report(const SaveTrace *trace, ReportWriter *writer) {
  ...
  write_report(writer,
               "save-trace enabled=%d attempts=%" PRIu64 " recorded=%" PRIu64
               " retained=%zu/%u"
               ...
               trace->retained, SAVE_TRACE_EVENT_CAPACITY, trace->overwritten,
               trace->refused_disabled, trace->refused_invalid,
               trace->truncated_labels);
  for (i = 0; i < SAVE_TRACE_POINT_COUNT; i++) {
    const SaveTracePointStats *stats = &trace->point[i];
    write_report(writer,
                 "point %s observed=%" PRIu64 "/%" PRIu64 " refused=%" PRIu64
                 " yes=%" PRIu64 " no=%" PRIu64 " unknown=%" PRIu64 "\n",
                 POINT_NAME[i], stats->recorded, stats->attempts,
                 stats->attempts - stats->recorded, stats->yes, stats->no,
                 stats->unknown);
  }
  for (i = 0; i < trace->retained; i++) {
    ...
    write_report(writer,
                 ...
                 " mode=%" PRIu32 " state=%" PRIu32 " device=%" PRIu32
                 " selection=%" PRIu32 " buffer=0x%08" PRIx32 " label=%s%s\n",
