...
static int same_state(PreviousState *previous, const void *state,
                      size_t bytes) {
  if (bytes > sizeof previous->bytes)
    bytes = sizeof previous->bytes;
...
int input_record_start(const char *path) {
  ...
  if (!path[0]) {
    ...
    snprintf(g_path, sizeof g_path,
             ...
             utc.tm_min, utc.tm_sec, (long)getpid());
  } else {
...
void input_record_keyboard(const void *state, size_t bytes, unsigned long frame,
  ...
  if (!g_file || !state || !bytes || same_state(&g_keyboard, state, bytes))
    return;
  if (bytes > KEYBOARD_BYTES)
    bytes = KEYBOARD_BYTES;
...
void input_record_mouse(const void *state, size_t bytes, unsigned long frame,
  ...
  if (!g_file || !state || !bytes || same_state(&g_mouse, state, bytes))
    return;
  if (bytes >= 4u)
    memcpy(&x, mouse, sizeof x);
  if (bytes >= 8u)
    memcpy(&y, mouse + 4u, sizeof y);
  if (bytes >= 12u)
    memcpy(&wheel, mouse + 8u, sizeof wheel);
  record_prefix("mouse", frame, guest_time_s);
  fprintf(g_file, "\"dx\":%d,\"dy\":%d,\"wheel\":%d,\"buttons\":", x, y, wheel);
