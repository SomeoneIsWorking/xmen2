...
static FILE *watch_out(void) {
  if (!w_out) {
    ...
    if (!w_out)
      w_out = stderr; /* better than nothing, if not by much */
  }
  return w_out;
}
...
static void watch_parse(void) {
  ...
  while (*s && w_n < WATCH_MAX_EPS) {
    ...
    if (end == s)
      break; /* not a number: stop, loudly */
    w_ep[w_n++] = (uint32_t)v;
    s = end;
    while (*s == ',' || *s == ' ')
      s++;
  ...
  if (*s)
    fprintf(watch_out(),
            "[WATCH] X2_WATCH has trailing junk that parsed to no "
            "address: \"%s\" -- those entry points are NOT watched\n",
            s);
  fprintf(watch_out(),
          "[WATCH] watching %d entry point(s), cap %d per point%s\n", w_n,
          w_cap, w_mem_enabled ? ", with a memory watch" : "");
  atexit(x86_watch_report);
}
...
static int watch_mem_change(uint32_t ep, uint32_t *address, uint32_t *value,
                            int *resolved) {
  uint32_t host;
  if (!w_mem_enabled)
    return 0;
  ...
  *resolved =
      x86_memwatch_read(&w_mem, host, watch_read32, NULL, address, value);
  ...
  if (*resolved)
    fprintf(watch_out(),
            "[MEM] before 0x%08x: chain 0x%08x resolved "
            "[0x%08x]=0x%08x CHANGED\n",
            ep, w_mem.root, *address, *value);
  else
    fprintf(watch_out(),
            "[MEM] before 0x%08x: chain 0x%08x UNRESOLVED "
...
void x86_watch_exit(uint32_t ep, const CPU *C) {
  ...
  if (w_all) {
    if (w_all_seen > (unsigned long)w_cap)
      return;
...
void x86_watch_enter(uint32_t ep, const CPU *C) {
  ...
  if (w_all) {
    if (++w_all_seen > (unsigned long)w_cap)
      return;
    fprintf(watch_out(),
            "[WATCH] 0x%08x ENTER  esp=0x%08x ecx=0x%08x "
            "ret=0x%08x arg0=0x%08x\n",
            ep, C->esp, C->ecx, C->esp ? RD32(C->esp) : 0u,
  ...
  if ((int)w_hits[i] > w_cap && !mem_changed)
    return;
  fprintf(watch_out(),
          "[WATCH] 0x%08x #%lu  esp=0x%08x ecx=0x%08x "
          "ret=0x%08x arg0=0x%08x",
          ep, w_hits[i], C->esp, C->ecx, C->esp ? RD32(C->esp) : 0u,
          C->esp ? RD32(C->esp + 4) : 0u);
  if (w_mem_enabled) {
    if (mem_resolved)
      fprintf(watch_out(), "  chain[0x%08x -> 0x%08x]=0x%08x%s", w_mem.root,
              mem_address, mem_value, mem_changed ? " CHANGED" : "");
    else
...
void x86_watch_stack(uint32_t ep, uint32_t guest_esp, const void *cpu,
  ...
  if (w_n < 0)
    watch_parse();
  x86_watch_stack_report(watch_out(), ep, guest_esp, (uint32_t)(uintptr_t)cpu,
                         cpu_size);
  fflush(watch_out());
}
...
void x86_watch_note_dump(FILE *o) { x86_watch_trace_dump(o); }

static void x86_watch_report(void) {
  int i;
  if (w_all) {
    fprintf(watch_out(),
            "[WATCH] %lu recompiled entry point call(s) total"
            " (printed the first %d)\n",
            w_all_seen, w_cap);
    fflush(watch_out());
    return;
  ...
  for (i = 0; i < w_n; i++) {
    if (w_hits[i])
      ...
      fprintf(watch_out(),
              "  0x%08x  NEVER CALLED -- this run says nothing "
              "about it\n",
              w_ep[i]);
  }
  fflush(watch_out());
