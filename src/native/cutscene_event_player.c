...
{
    return x86_peek(owner + HEAP + index * HEAP_ENTRY_BYTES,
                    entry, sizeof *entry);
}
                              ...
                              uint32_t inflight_slot)
{
    ...
    if (extra) {
        if (mask_has(expected, inflight_slot)) return 0;
...
static int validate_events_with_inflight(uint32_t owner,
                                         uint32_t inflight_slot,
...
{
    return validate_events_with_inflight(owner,
        CUTSCENE_EVENT_PLAYER_CAPACITY, events);
}
...
static void repair_heap(uint32_t owner, uint32_t count, uint32_t index)
{
    ...
    while (index != 0u) {
        ...
        if (!(deadline_value(&here) < deadline_value(&other))) break;
    ...
    for (;;) {
        ...
        if (!(deadline_value(&other) < deadline_value(&here))) break;
                          ...
                          const ValidatedEvents *events, uint32_t index)
{
    ...
    ++causal_depth;
    if (owned) ++owned_execution_depth;
    ...
    call_guest(cpu, base + (FN_SLOT_FREE - EXE_PREFERRED), events->owner,
               1, slot);
}
static CutsceneEventPlayerStep step_due(CPU *cpu, uint32_t base,
                                        uint32_t owner, float now)
{
    ValidatedEvents events;
    if (!validate_events(owner, &events))
        return CUTSCENE_EVENT_PLAYER_STEP_REFUSED;
    if (events.count == 0u ||
        !(deadline_value(&events.entry[0]) < now))
...
int cutscene_event_player_window_begin(CutsceneEventOwnershipWindow *window)
{
    ...
    if (!window) return -1;
    if (!owner) return 0;
    if (!validate_events(owner, &events)) return -1;
    ...
    CutsceneEventInsertionOwner owns_current, void *opaque)
{
    ValidatedEvents events;
    uint32_t owner;
    if (!window || !owns_current) return -1;
...
void cutscene_event_player_unwatch_insertions(CutsceneEventOwnershipWindow *window)
{
    ...
    const CutsceneEventOwnershipWindow *window, uint32_t *slot)
{
    ...
    for (index = 0; index < events.count; ++index) {
        float deadline;
        if (!mask_has(window->owned, events.entry[index].slot)) continue;
    ...
    if (selected == CUTSCENE_EVENT_PLAYER_CAPACITY) return 0;
...
CutsceneEventPlayerStep cutscene_event_player_step_owned(
    CPU *cpu, CutsceneEventOwnershipWindow *window)
{
    uint32_t slot;
    int found = cutscene_event_player_next_owned(window, &slot);
    if (found < 0) return CUTSCENE_EVENT_PLAYER_STEP_REFUSED;
    if (found == 0) return CUTSCENE_EVENT_PLAYER_STEP_NONE;
...
{
    atomic_fetch_add_explicit(&insertion_faults, 1u,
                              memory_order_relaxed);
    if (window) window->active = 0u;
}
                               ...
                               uint32_t *new_slot)
{
    ...
    if (after->count != before->count + 1u) return 0;
    for (before_index = 0; before_index < before->count; ++before_index) {
        ...
        for (after_index = 0; after_index < after->count; ++after_index)
            if (before->entry[before_index].deadline_bits ==
                ...
                before->entry[before_index].slot ==
                    after->entry[after_index].slot) {
                found = 1;
                break;
            }
        if (!found) return 0;
    ...
    for (after_index = 0;
         after_index < CUTSCENE_EVENT_PLAYER_SLOT_WORDS; ++after_index) {
        uint32_t added = after->active[after_index] &
                         ~before->active[after_index];
        while (added) {
            uint32_t bit = 0;
            uint32_t bits = added;
            while ((bits & 1u) == 0u) {
    ...
    return new_count == 1u &&
           *new_slot < CUTSCENE_EVENT_PLAYER_CAPACITY;
}
void x2_override_004b2b40(CPU *cpu)
{
    ...
    if (!cpu || !(causal_depth
        ...
        if (cpu) cpu->esp += 12u; /* Refused RET 8, without guest mutation. */
        return;
    }
    owner = cpu->ecx;
...
void x2_override_004b2d70(CPU *cpu)
{
    ...
    if (cpu) cpu->esp += 8u; /* RET 4: return address plus float argument. */
}
__attribute__((constructor))
static void x2_cutscene_event_player_register_override(void)
{
    x86_register_override("XMen2.exe", FN_EVENT_INSERT,
                          x2_override_004b2b40);
    x86_register_override("XMen2.exe", FN_EVENT_PUMP,
                          x2_override_004b2d70);
}
