...
int dinput_pad_virtual_release(const char *what)
{
#ifdef X2_WITH_SDL
    int i;
    if (!g_virt_js || !what) return 0;
    for (i = 0; i < X2_VIRTUAL_BUTTON_COUNT; i++) {
        if (!strcmp(what, g_vbtn_name[i])) {
            if (!SDL_SetJoystickVirtualButton(g_virt_js, i, false)) return 0;
            g_vbtn_until[i] = 0.0;
            SDL_UpdateJoysticks();
            SDL_UpdateGamepads();
            return 1;
    ...
    if (!strcmp(what, "up") || !strcmp(what, "down") ||
        ...
        if (!SDL_SetJoystickVirtualHat(g_virt_js, 0, SDL_HAT_CENTERED)) return 0;
        SDL_UpdateJoysticks();
        SDL_UpdateGamepads();
        return 1;
    }
    for (i = 0; i < X2_VIRTUAL_AXIS_COUNT; i++) {
        if (!strcmp(what, g_vaxis_name[i])) {
            ...
            if (!SDL_SetJoystickVirtualAxis(g_virt_js, i, rest)) return 0;
