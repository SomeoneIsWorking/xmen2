...
static int table_matches_live_inventory(void)
{
    ...
    for (slot = 0; slot < DINPUT8_CONTROLLER_SLOTS; slot++)
        if (dinput8_controller_host_pad_for_slot(slot) >= 0) matched++;
...
void dinput8_check_controller_table(void)
{
    ...
    if (!g_pad_enum || !g_pad_ref) return;
    broken = !table_matches_live_inventory();
    if (!broken) { was_broken = 0; return; }
    if (was_broken) return;
    was_broken = 1;
    fprintf(stderr, "DINPUT8: the game's controller table names NO live pad "
...
{
    x2_controller_hotplug_enumerated(&g_hotplug, generation, connected,
                                     reported);
}

unsigned long dinput8_hotplug_admissions(void)
{
    return g_hotplug.admissions;
}
