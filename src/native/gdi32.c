...
static struct { int used; uint32_t bitmap, textcolor, bkmode; } g_dc[MAX_DC];
static struct { int used; uint32_t bits, bytes, w, h, bpp; } g_bmp[MAX_BMP];
static unsigned long g_dcs, g_bmps, g_text_ignored;

static int dc_index(uint32_t h)
{
    if ((h & 0xFF000000u) != DC_TAG) return -1;
    { uint32_t i = (h & 0xFFFFFFu); return (i && i <= MAX_DC && g_dc[i - 1].used)
                                          ? (int)i - 1 : -1; }
}

static int bmp_index(uint32_t h)
{
    if ((h & 0xFF000000u) != BMP_TAG) return -1;
    { uint32_t i = (h & 0xFFFFFFu); return (i && i <= MAX_BMP && g_bmp[i - 1].used)
                                          ? (int)i - 1 : -1; }
}
...
{
    int i;
    for (i = 0; i < MAX_DC; i++) if (!g_dc[i].used) break;
    if (i == MAX_DC) {
        fprintf(stderr, "GDI32: all %d memory DCs are live; CreateCompatibleDC "
...
void imp_GDI32_CreateDIBSection(CPU *C)
{
    ...
    if (!pbmi || !ppv) { ret_std(C, 0, 6); return; }
    if (hsection) {
        /* A DIB backed by a file mapping is a different object -- the pixels
           ...
           would not see. */
        fprintf(stderr, "GDI32: CreateDIBSection with a SECTION handle "
    ...
    if (compression != 0u) {                   /* BI_RGB only */
        fprintf(stderr, "GDI32: CreateDIBSection with compression %u; only "
    ...
    if (w <= 0 || h == 0 || (bpp != 8u && bpp != 16u && bpp != 24u && bpp != 32u)) {
        fprintf(stderr, "GDI32: CreateDIBSection(%d x %d, %u bpp) -- refusing a "
    ...
    for (i = 0; i < MAX_BMP; i++) if (!g_bmp[i].used) break;
    if (i == MAX_BMP) {
...
void imp_GDI32_SetTextColor(CPU *C)
{
    int i = dc_index(A(0));
    uint32_t prev;
    if (i < 0) { ret_std(C, 0xFFFFFFFFu, 2); return; }   /* CLR_INVALID */
...
{
    int i, dcs = 0, bmps = 0;
    for (i = 0; i < MAX_DC; i++) if (g_dc[i].used) dcs++;
    for (i = 0; i < MAX_BMP; i++) if (g_bmp[i].used) bmps++;
