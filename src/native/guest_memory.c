...
static int span(uint32_t address, size_t size, uint32_t *first, uint32_t *count)
{
    uint64_t start = align_down(address);
    uint64_t end = align_up((uint64_t)address + size);
    if (!size || end > GUEST_SPACE_SIZE || end <= start) return -1;
...
static int apply_host_protection(uint32_t first, uint32_t count)
{
    ...
    uint32_t end = (first + count + pages_per_host - 1u)
                 & ~(pages_per_host - 1u);
...
int guest_memory_host_address(const void *pointer, uint32_t *address)
{
    ...
    if (host < g_guest_memory_base ||
        ...
        return 0;
    if (address) *address = (uint32_t)(host - g_guest_memory_base);
    return 1;
}
...
int guest_memory_map_fixed(uint32_t address, size_t size, int protection)
{
    ...
    void *host;
    uint64_t start;
    if (!g_ready && guest_memory_init() != 0) return -1;
    ...
    if (host == MAP_FAILED || (uintptr_t)host != start) {
        if (host != MAP_FAILED) munmap(host, (size_t)count * GUEST_PAGE_SIZE);
        pthread_mutex_unlock(&g_pages_lock);
        errno = EEXIST;
        return -1;
    }
                         ...
                         uint32_t *address)
{
    ...
    if (!alignment) alignment = GUEST_PAGE_SIZE;
...
int guest_memory_is_readable(uint32_t address, size_t size)
{
    uint32_t first, count, i;
    int readable = 1;
    if (span(address, size, &first, &count) != 0) return 0;
    pthread_mutex_lock(&g_pages_lock);
    for (i = 0; i < count; i++)
        if (!(g_pages[first + i] & PAGE_MAPPED) ||
