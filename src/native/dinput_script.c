...
static void script_parse(void)
{
    ...
    for (p = value; *p; ) {
        ...
        if (sscanf(p, "%lf-%lf/%lf+%lf:%23[^ ,\t]%n",
                   &at, &to, &step, &hold, name, &consumed) == 5 ||
            (consumed = 0, sscanf(p, "%lf-%lf/%lf:%23[^ ,\t]%n",
                                  &at, &to, &step, name, &consumed)) == 4) {
            if (step <= 0.0 || to < at) {
        ...
        } else if (sscanf(p, "%lf+%lf:%23[^ ,\t]%n",
                          &at, &hold, name, &consumed) != 3 &&
                   (consumed = 0, sscanf(p, "%lf:%23[^ ,\t]%n",
                                         &at, name, &consumed)) != 2) {
        ...
        }
        {
            ...
            if (!dik) {
                fprintf(stderr, "DINPUT8: \"%s\" has no DirectInput DIK "
            ...
            if (repeats > 1)
                fprintf(stderr, "DINPUT8: X2_INPUT_SCRIPT -- \"%s\" repeats "
                        "%d time(s), every %g %s from %g to %g.\n",
                        name, repeats, step,
                        by_frame ? "frame(s)" : "second(s)", at,
