...
int x2_win32_message_post(X2Win32Mouse *mouse, const X2Win32Message *message) {
  if (!mouse || !message || mouse->message_count >= X2_WIN32_MESSAGE_CAPACITY)
    return 0;
...
int x2_win32_message_take(X2Win32Mouse *mouse, uint32_t hwnd,
                          uint32_t filter_min, uint32_t filter_max, int remove,
                          X2Win32Message *message) {
  size_t i;
...
int x2_win32_mouse_map_point(int32_t x, int32_t y, uint32_t window_width,
...
int x2_win32_mouse_motion(X2Win32Mouse *mouse, uint32_t hwnd, int32_t client_x,
  ...
  if (!mouse)
    return 0;
  mouse->buttons = buttons & (X2_MK_LBUTTON | X2_MK_RBUTTON | X2_MK_MBUTTON);
  message.hwnd = hwnd;
  ...
  if (mouse->message_count != 0u &&
      ...
      mouse->messages[mouse->message_count - 1u].message == X2_WM_MOUSEMOVE) {
...
static int button_messages(X2Win32MouseButton button, int down, uint32_t *flag,
                           uint32_t *message) {
  switch (button) {
...
int x2_win32_mouse_button(X2Win32Mouse *mouse, uint32_t hwnd,
                          X2Win32MouseButton button, int down, int32_t client_x,
                          int32_t client_y, int32_t screen_x, int32_t screen_y,
...
void x2_win32_mouse_window_state(X2Win32Mouse *mouse, int hidden, int focused,
                                 int pointer_inside) {
  if (!mouse)
    return;
