...
static int context_index(uint32_t manager, uint32_t context,
                         uint32_t *index)
{
    uint32_t pool = manager + MANAGER_CONTEXT_POOL;
    uint32_t distance;
    if (context < pool) return 0;
...
static int validate_heap(uint32_t scheduler, ValidatedHeap *heap)
{
    ...
    for (index = 0; index < count; ++index) {
        ...
        if (!read_entry(scheduler, index, entry) ||
            entry->slot >= MAX_CONTEXTS ||
            (used_slots & (1u << entry->slot)) != 0u ||
            isnan(entry_deadline(entry)) ||
            !x86_peek32(scheduler + SCHEDULER_CONTEXT_SLOTS +
                            entry->slot * 4u,
                        &context) ||
...
static void repair_heap(uint32_t scheduler, uint32_t count, uint32_t index)
{
    ...
    while (index != 0u) {
        ...
        if (!(entry_deadline(&here) < entry_deadline(&other))) break;
    ...
    for (;;) {
        ...
        if (!(entry_deadline(&other) < entry_deadline(&here))) break;
                                       ...
                                       uint32_t index)
{
    ...
    if ((completed & 0xffu) == 1u) {
        ...
        if (live_manager == heap->manager)
            ...
            call_guest(cpu,
                       base + (FN_SCHEDULER_SLOT_RELEASE - EXE_PREFERRED),
...
static BehavedPlayerStep step_due(CPU *cpu, uint32_t base,
                                  uint32_t scheduler, float now)
{
    ValidatedHeap heap;
                              ...
                              void *opaque, uint32_t *context)
{
    ...
    if (!cpu || !owns || !context || !(base = exe_base()) ||
        !(owner = manager(cpu, base)) ||
        owner > UINT32_MAX - MANAGER_SCHEDULER ||
        !validate_heap(owner + MANAGER_SCHEDULER, &heap) ||
        heap.manager != owner)
        return -1;
    for (index = 0; index < heap.count; ++index) {
        float deadline;
        if (!owns(heap.context[index], opaque)) continue;
    ...
    if (selected == MAX_CONTEXTS) return 0;
...
BehavedPlayerStep behaved_player_step_owned(
    CPU *cpu, BehavedPlayerOwnsContext owns, void *opaque)
{
    uint32_t context;
    int found = behaved_player_next_owned(cpu, owns, opaque, &context);
    if (found < 0) return BEHAVED_PLAYER_STEP_REFUSED;
    if (found == 0) return BEHAVED_PLAYER_STEP_NONE;
...
static void x2_behaved_player_register_override(void)
{
    /* The generated fn_XMen2_004d9640 remains linked for differential A/B. */
    x86_register_override("XMen2.exe", FN_SCHEDULER_PUMP,
                          x2_override_004d9640);
}
