 ...
 * through COM vtables on the objects it hands back. libIGDisplay calls it
 * twice, from igWin32Window::userConstruct and
 * igWin32ControllerManager::initialize, both with dwVersion 0x700, and both
 * immediately dispatch vtable slot 4 (EnumDevices) with an enumeration callback
 * of their own.
 *
...
static const char *const VT_NAME[VT_COUNT] = {
    ...
    "EnumDevices",    "GetDeviceStatus", "RunControlPanel", "Initialize",
    "FindDevice",     "CreateDeviceEx"};
...
static void m_EnumDevices(CPU *C) {
  ...
  for (i = 0; i < 2; i++) {
    unsigned t =
        KINDS[i] == DINPUT_DEV_KEYBOARD ? DIDEVTYPE_KEYBOARD : DIDEVTYPE_MOUSE;
    uint32_t inst;
    CPU K;
    if (wanted != 0 && wanted != t)
      continue; /* 0 means ALL */
...
static void create_device(CPU *C, uint32_t guid, uint32_t out, uint32_t outer,
  ...
  if (!out) {
    ret_com(C, DIERR_INVALIDPARAM, nargs);
    return;
  }
  WR32(out, 0);
  if (outer || !guid) {
...
static void build(void) {
  static void (*const impl[VT_COUNT])(CPU *) = {
      ...
      m_CreateDevice,    m_EnumDevices, m_GetDeviceStatus,
      m_RunControlPanel, m_Initialize,  NULL, /* FindDevice */
      m_CreateDeviceEx};
  int k;
  if (g_object)
    return;
