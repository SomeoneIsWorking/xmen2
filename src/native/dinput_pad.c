                            ...
                            const SDL_GUID *guid, unsigned ordinal)
{
       ...
       persisted-assignment resolution a stored controller0 id goes through. */
    {
        ...
        if (override) {
            snprintf(p->persistent_id, sizeof p->persistent_id, "%s",
                     override);
            return;
        }
    }
...
int dinput_pad_open_gamepad_button(int pad, int gamepad_button)
{
    ...
    if (!p || !p->gp || gamepad_button < 0
        ...
        return -1;
    return SDL_GetGamepadButton(p->gp, (SDL_GamepadButton)gamepad_button)
               ? 1 : 0;
#else
    (void)pad; (void)gamepad_button;
    return -1;
#endif
}
