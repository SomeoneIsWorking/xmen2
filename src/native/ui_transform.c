...
static int read_matrix(uint32_t guest, float out[16])
{
    ...
    for (i = 0; i < 16; i++) {
        uint32_t bits;
        if (!x86_peek32(guest + i * 4u, &bits)) return 0;
...
{
    if (which == UI_PROJECTION) return 1u;
    if (which == UI_WORLD) return 2u;
    if (which == UI_VIEW) return 4u;
    return 0u;
}

static void select_context(uint32_t context)
{
    if (g_have_context && g_context == context) return;
    g_context = context;
    g_have_context = 1;
    g_valid = 0;
...
static void x2_ui_transform_register(void)
{
