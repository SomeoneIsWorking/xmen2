...
int x2_boot_player_select_primary(CPU *source, unsigned primary_player)
{
    ...
    if (!source || primary_player >= LOCAL_PLAYER_COUNT) return 0;
    ...
    manager = call.eax;
    if (!manager || !x86_peek32(RD32(manager) + PAD_SET_CURRENT_PLAYER,
                                &setter) || !setter)
        return 0;
    ...
    if (RD32(manager + PAD_CURRENT_PLAYER) != primary_player) {
        fprintf(stderr, "BOOT PLAYER: CPadManager rejected primary local "
                        "player %u.\n", primary_player + 1u);
        return 0;
    }
    fprintf(stderr, "BOOT PLAYER: title/menu presentation was bypassed; "
