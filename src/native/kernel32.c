...
void imp_KERNEL32_SetLastError(CPU *C) {
  g_last_error = A(0);
  ret_std(C, 0, 1);
}
...
void imp_KERNEL32_GetCurrentProcessId(CPU *C) {
  ret_std(C, (uint32_t)getpid(), 0);
}
...
void imp_KERNEL32_GetSystemInfo(CPU *C) {
  ...
  if (n < 1)
    n = 1;
  if (n > 32)
    n = 32; /* dwActiveProcessorMask is 32 bits */
...
__attribute__((constructor)) static void kernel32_runtime_exports(void) {
...
void imp_KERNEL32_GetFileSize(CPU *C) {
  ...
  if (A(1))
    WR32(A(1), (uint32_t)((uint64_t)st.st_size >> 32));
...
static void sync_name(Handle *hh, uint32_t namep) {
  if (namep)
    ...
    snprintf(hh->name, sizeof hh->name, "%s", "(unnamed)");
}
...
void imp_KERNEL32_CreateDirectoryA(CPU *C) {
  ...
  k32_file_trace("CreateDirectory", g, p, ok ? "created" : strerror(errno));
...
void imp_KERNEL32_GetSystemDirectoryA(CPU *C) {
  ...
  if (A(1) > n)
    memcpy(guest_memory_pointer(A(0)), s, n + 1);
  ret_std(C, n, 2);
}
...
static uint32_t module_base_by_name(const char *want) {
  ...
  for (m = x86_modules(); m; m = m->next) {
    if (strcasecmp(m->name, base) == 0)
      return *m->base;
    if (strncasecmp(m->name, base, n) == 0 && m->name[n] == '.')
      return *m->base;
  }
  return 0;
}
...
static const char *sysmod_name(uint32_t h) {
  uint32_t i;
  if (h < SYSMOD_BASE ||
      h >= SYSMOD_BASE + (uint32_t)MAX_SYSMOD * SYSMOD_STRIDE)
    return NULL;
...
static int is_native_sysmod(const char *n) {
  static const char *known[] = {"KERNEL32.DLL", "USER32.DLL", "GDI32.DLL",
                                "ADVAPI32.DLL", "MSVCRT.DLL", "MSVCR71.DLL",
                                "SHELL32.DLL",  "OLE32.DLL",  NULL};
  int i;
  if (!n)
    return 0;
  for (i = 0; known[i]; i++)
    if (strcasecmp(known[i], n) == 0)
      return 1;
...
void imp_KERNEL32_CreateFileW(CPU *C) {
  k32_unimpl("CreateFileW",
             "the wide-character file API is unused by the "
...
static int find_next(Handle *hh, uint32_t data) {
  ...
  while ((e = readdir(hh->dir)) != NULL) {
    if (!win_match(hh->pattern, e->d_name))
      continue;
    if (fd_fill(data, hh->dirpath, e->d_name))
      return 1;
  }
  return 0;
}
...
void imp_KERNEL32_FindFirstFileA(CPU *C) {
  ...
  if (!find_next(hh, data)) {
    k32_file_trace("FindFirstFile", spec, hh->dirpath, "matched NOTHING");
    closedir(hh->dir);
    hh->dir = NULL;
    hh->kind = 0;
    g_last_error = ERROR_FILE_NOT_FOUND;
  ...
  k32_file_trace("FindFirstFile", spec, hh->dirpath, "matched at least one");
  ret_std(C, h, 2);
}
...
void imp_KERNEL32_MultiByteToWideChar(CPU *C) {
  ...
  for (i = 0; i < n; i++)
    WR16(dst + (uint32_t)i * 2u, cp1252_to_utf16(s[i]));
  ret_std(C, (uint32_t)n, 6);
}
...
void imp_KERNEL32_LocalFree(CPU *C) {
  guest_free(A(0));
  ret_std(C, 0, 1);
}
...
void imp_KERNEL32_GlobalMemoryStatus(CPU *C) {
  ...
  WR32(p + 12u, (uint32_t)(X2_PHYS_BYTES - g_reserved_bytes > X2_PHYS_BYTES
                               ? 0
                               : X2_PHYS_BYTES - g_reserved_bytes)); /* avail */
  WR32(p + 16u, (uint32_t)X2_PHYS_BYTES); /* dwTotalPageFile */
  WR32(p + 20u, (uint32_t)(X2_PHYS_BYTES - g_reserved_bytes > X2_PHYS_BYTES
                               ? 0
...
void imp_KERNEL32_SetFilePointer(CPU *C) {
  ...
  off_t r;
  if (phi)
    off |= (int64_t)(int32_t)RD32(phi) << 32;
  r = lseek(hh->fd, off,
            method == 1   ? SEEK_CUR
            : method == 2 ? SEEK_END
                          : SEEK_SET);
  ...
  if (phi)
    WR32(phi, (uint32_t)((uint64_t)r >> 32));
...
void imp_KERNEL32_HeapSize(CPU *C) {
  uint32_t base, size;
  if (!guest_heap_contains(A(2), &base, &size)) {
    ret_std(C, 0xFFFFFFFFu, 3);
    return;
  }
...
void imp_KERNEL32_IsValidCodePage(CPU *C) {
  uint32_t cp = A(0);
  ret_std(
      C, (cp == CP_ACP_US || cp == CP_OEM_US || cp == 0u || cp == 1u) ? 1u : 0u,
      1);
}
...
static const char *locale_field(uint32_t lctype) {
  ...
  case 0x0002:
    return "en-US"; /* LOCALE_SLOCALIZEDDISPLAYNAME-ish */
  case 0x0004:
    return "1252"; /* LOCALE_IDEFAULTANSICODEPAGE */
  case 0x000B:
    return "437"; /* LOCALE_IDEFAULTCODEPAGE */
  case 0x0005:
    return "ENU"; /* LOCALE_SABBREVLANGNAME */
  case 0x000E:
    return "."; /* LOCALE_SDECIMAL */
  case 0x000F:
    return ","; /* LOCALE_STHOUSAND */
  case 0x0059:
    return "en"; /* LOCALE_SISO639LANGNAME */
  case 0x005A:
    return "US"; /* LOCALE_SISO3166CTRYNAME */
  default:
    return NULL;
  }
}
...
static void lcmap(CPU *C, int wide) {
  ...
  if (wide) {
    ...
    for (i = 0; i < nsrc; i++) {
      int c = s[i];
      if (c < 128)
        c = (flags & LCMAP_UPPERCASE)   ? toupper(c)
            : (flags & LCMAP_LOWERCASE) ? tolower(c)
                                        : c;
      d[i] = (uint16_t)c;
    }
  } else {
