...
static void put(char *out, size_t size, size_t *at, const char *format, ...)
{
    ...
    va_end(args);
    if (written > 0)
        *at += (size_t)written < size - *at
            ? (size_t)written : size - *at;
}
...
static uint32_t thiscall(const CPU *source, uint32_t function,
                         uint32_t object, int has_argument, uint32_t argument)
{
    CPU call = *source;
    if (has_argument) {
        call.esp -= 4u;
...
static void report_pad_players(CPU *cpu, uint32_t base, char *out,
                               size_t size, size_t *at)
{
    ...
    if (!manager) {
        put(out, size, at, "pad manager: not constructed; no player input "
                           "state read\n");
        return;
    }
    ...
    for (player = 0; player < players; player++) {
        ...
        if (!x86_peek32(vtable + PAD_PLAYER, &function) || !function) continue;
        ...
        put(out, size, at, "  player %u object 0x%08x logical mask 0x%08x "
                           "physical:", player, object, mask);
