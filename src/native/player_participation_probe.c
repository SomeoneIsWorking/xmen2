...
static void report_pad_players(CPU *cpu, uint32_t base, char *out, size_t size,
  ...
  if (!manager) {
    text_put(out, size, at,
             "pad manager: not constructed; no player input "
             "state read\n");
    return;
  }
  if (!guest_memory_try_read32(manager, &vtable) ||
      !guest_memory_try_read32(vtable + PAD_COUNT, &function) || !function) {
    text_put(out, size, at, "pad manager 0x%08x: player count unreadable\n",
             manager);
    return;
  }
  players = thiscall(cpu, function, manager, 0, 0u);
  text_put(out, size, at, "pad manager 0x%08x: %u player input object(s)\n",
           manager, players);
  if (players > 4u)
    players = 4u;
  for (player = 0; player < players; player++) {
    uint32_t object, player_vtable = 0, mask = 0, index, nonzero = 0;
    if (!guest_memory_try_read32(vtable + PAD_PLAYER, &function) || !function)
      continue;
    object = thiscall(cpu, function, manager, 1, player);
    if (!object) {
      text_put(out, size, at, "  player %u: no input object\n", player);
      continue;
    }
    if (guest_memory_try_read32(object, &player_vtable) &&
        guest_memory_try_read32(player_vtable + PLAYER_MASK, &function) &&
        function)
    ...
    for (index = 0; index < PLAYER_PHYSICAL_COUNT; index++) {
      uint32_t bits = 0;
      float value;
      if (!guest_memory_try_read32(object + PLAYER_PHYSICAL + index * 4u,
                                   &bits))
        continue;
      memcpy(&value, &bits, sizeof value);
      if (value != 0.0f) {
        text_put(out, size, at, " [%u]=%.3f", index, (double)value);
        nonzero++;
      }
    }
    text_put(out, size, at, "%s\n", nonzero ? "" : " 0 of 30 floats non-zero");
  }
}
...
size_t x2_player_participation_probe_report(CPU *cpu, char *out, size_t size) {
  ...
  if (!manager || !RD32(manager)) {
    ...
    text_put(out, size, &at,
             ...
             mask & 2u ? "active" : "inactive",
             mask & 4u ? "active" : "inactive",
