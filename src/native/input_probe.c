...
#define RESOLVE_HANDLE_RVA                                                     \
  RVA(0x004654b0u) /* FUN_004654b0(this=&handle) -> actor */
 ...
 */
#define DI_WRAPPER_RVA                                                         \
  RVA(0x00a6abfcu) /* [0x00a6abfc], the DirectInput wrapper */
...
static const char *pad_code_name(uint32_t code) {
  ...
  if (code >= 1u && code <= 0x10u)
    return AXES[code - 1u];
  if (code >= 0x11u && code <= 0x14u)
