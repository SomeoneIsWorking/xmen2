...
static uint32_t guest_call(const CPU *source, uint32_t target, uint32_t self,
                           const uint32_t *arguments, size_t count,
...
static uint32_t resolve_argument(const CPU *cpu, uint32_t base,
  ...
  if (root != INVALID_INDEX) {
    ...
    if (raw < split)
      index = find_value(cpu, base, context + 8u, raw, RD32(branch + 0x10u));
    else if (raw > split)
      index = find_value(cpu, base, context + 8u, raw, RD32(branch + 0x14u));
  }
...
static void assign_result(CPU *cpu, uint32_t base, uint32_t context,
  ...
  key = script_index == INVALID_INDEX
            ? 0u
            : RD32(script + 0x1acu + script_index * 8u);
  context_index =
      find_value(cpu, base, context + 8u, key, RD32(context + 0x0cu));
  if (context_index == INVALID_INDEX)
    return;
  destination = RD32(context + 0x170u + context_index * 4u);
  if (!destination)
    return;
  guest_call(cpu, RD32(RD32(destination) + 0x18u), destination, arguments, 1u,
             4u);
}

static int result_is_true(CPU *cpu, uint32_t result) {
  return guest_call(cpu, RD32(RD32(result) + 0x10u), result, NULL, 0u, 0u) ==
         1u;
}
...
static void release_result(CPU *cpu, uint32_t base, uint32_t result) {
  ...
  if (!result)
    return;
  manager = guest_call(cpu, linked(base, FN_MANAGER), 0u, NULL, 0u, 0u);
  pool = manager + VALUE_POOL;
  if (result < pool)
    return;
  distance = result - pool;
  if (distance % VALUE_STRIDE != 0u)
    return;
  ...
  while (node) {
    ...
    if (RD8(node + 0x34u) == 1u) {
      if (next)
        WR32(context, next);
      next = 0u;
    } else if (RD8(node + 0x35u) == 1u && !result_is_true(cpu, result)) {
