...
static unsigned long c_upd_calls, c_upd_disabled, c_upd_invisible,
    c_upd_applied, c_upd_ended, c_upd_nolines, c_upd_gate_seen,
    c_upd_gate_pressed, c_upd_gate_debounced, c_upd_advanced;
...
void x2_override_00455600(CPU *C) {
  ...
  c_launch_asked++;
  if (name_is_empty(name, "script name"))
    c_launch_empty++;
  ...
  for (slot = 0; slot < (int)RD32(self + CV_RESP_COUNT); slot++) {
    ...
    if (filled == sel) {
      if (!((uint8_t)call1(C, FN_BEGIN_RESPONSE, self, id))) {
        uint32_t line = call1(C, FN_LINE_BY_ID, self, RD32(self + CV_CUR_LINE));
        if (line) {
...
static uint32_t cdecl_call(CPU *C, uint32_t fn, int argc,
                           const uint32_t *argv) {
  CPU K = *C;
  int i;
  K.esp -= (uint32_t)argc * 4u;
...
static long double call_float(CPU *C, uint32_t fn, uint32_t ecx, int argc,
                              const uint32_t *argv) {
  CPU K = *C;
  int i;
  K.esp -= (uint32_t)argc * 4u;
...
void x2_override_0045d1a0(CPU *C) {
  ...
  {
    uint32_t arg = 1u;
    cdecl_call(C, FN_ENTER_UI, 1, &arg);
  }
  ...
  if (RD32(self + 0x239a0u)) {
    ...
    if ((uint8_t)call1(C, vslot(au, 0x60u), au, RD32(sing + CV_SOUND_HANDLE))) {
  ...
  c_upd_gate_seen++;
  {
    ...
    if (advance) {
      ...
      if (RD32(self + CV_SOUND_HANDLE) != RD32(G_NULL_HANDLE)) {
        ...
        WR8(self + CV_ACCEPT_STATE, RD8(self + CV_ACCEPT_STATE) & 0xfeu);
        WR32(self + CV_SOUND_HANDLE, RD32(G_NULL_HANDLE));
      }
  ...
  /* 0x0045d3bf: move the highlight with the stick / d-pad. */
  {
    ...
    if (vertical < 0) {
      ...
      WR16(self + CV_TAG_WRAP, (uint16_t)(w < 0 ? 0 : w));
      {
        ...
        if (t < 0)
          t = (int16_t)(RD16(self + CV_TAG_COUNT) - 1);
        WR16(self + CV_TAG_INDEX, (uint16_t)t);
      }
      call0(C, FN_AFTER_CHOICE, self);
    } else if (vertical > 0) {
      int16_t t = (int16_t)(RD16(self + CV_TAG_INDEX) + 1);
      if (t >= (int16_t)RD16(self + CV_TAG_COUNT))
        t = 0;
      WR16(self + CV_TAG_INDEX, (uint16_t)t);
      {
        int16_t w = (int16_t)(RD16(self + CV_TAG_WRAP) + 1);
...
x2_conversation_register_overrides(void) {
