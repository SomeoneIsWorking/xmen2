                             ...
                             char *out, int cap)
{
    RegValue *v;
    advapi32_store_load();
    if (!path || !name || !out || cap <= 0) return 0;
    v = advapi32_store_find(path, name);
    if (!v || v->type != 1u /* REG_SZ */ || v->len >= (uint32_t)cap) return 0;
                             ...
                             const char *value)
{
    ...
    size_t n;
    advapi32_store_load();
    if (!path || !name || !value) return 0;
    n = strlen(value);
    if (n >= MAX_DATA_) {
        fprintf(stderr, "advapi32: host publication of \"%s|%s\" is %d bytes "
