...
bool copy_path(const std::filesystem::path &source, char *destination,
  ...
  if (!destination || text.empty() || text.size() >= capacity)
    return false;
...
bool clean_tree(const std::filesystem::path &path, std::string &error) {
  ...
  if (!status)
    return true;
  error = "cannot clean " + path.filename().string() + ": " + status.message();
  return false;
}
...
bool restore_interrupted_swap(const std::filesystem::path &destination,
  ...
  if (status) {
    error = "cannot inspect the previous ZIP extraction: " + status.message();
    return false;
  }
...
bool accept_preparation(const std::filesystem::path &preparing,
  ...
  if (status) {
    error = "cannot preserve the previous ZIP extraction: " + status.message();
    return false;
  }
  ...
  if (status) {
    ...
    error = "cannot accept the prepared ZIP extraction: " + status.message();
    if (restore_error)
