...
static int stop_active_voice(CPU *cpu, uint32_t manager)
{
    ...
    WR8(manager + CV_ACCEPT_STATE,
        RD8(manager + CV_ACCEPT_STATE) & 0xfeu);
...
int cutscene_dialogue_advance(struct CPU *cpu)
{
    ...
    cutscene_dialogue_skip_end(cpu);
    {
        ...
        if (playing > 0) {
            g_dialogue.counters.skip_presentation_starts++;
            if (!stop_active_voice(cpu, selection.manager)) return 0;
        } else if (playing < 0) {
            return 0;
        }
    }
...
static void x2_cutscene_dialogue_register_override(void)
{
    x86_register_override("XMen2.exe", FN_BEGIN_RESPONSE,
                          x2_override_00458700);
    x86_register_override("XMen2.exe", FN_LINE_AUDIO,
                          x2_override_0045a170);
}
