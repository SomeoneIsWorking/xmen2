 ...
 * chosen.  The player queries that yield here and resumes it through the same
 * vtable +0x18 chooseResponse transition used by retail input.
 */
...
ConversationPlayerState conversation_player_state(struct CPU *cpu)
{
    ...
    if (!(flags & CVF_VISIBLE)) return CONVERSATION_PLAYER_INACTIVE;
    if (!x86_peek32(self + CV_RESP_COUNT, &count) ||
        count > RESPONSE_SLOTS)
        return CONVERSATION_PLAYER_UNREADABLE;
    for (i = 0; i < count; i++) {
        if (!x86_peek32(self + CV_RESP_IDS + i * 4u, &id))
            return CONVERSATION_PLAYER_UNREADABLE;
        if (id != RESPONSE_NONE) responses++;
    }
    if (responses == 1u) return CONVERSATION_PLAYER_DETERMINISTIC;
    if (responses > 1u) return CONVERSATION_PLAYER_CHOICE;
...
int conversation_player_advance(struct CPU *cpu)
{
    ...
    if (!cpu || conversation_player_state(cpu) !=
    ...
    call = *cpu;
    call.esp -= 4u;
    WR32(call.esp, selected);
    call.ecx = self;
    x86_guest_call_args(&call, function, 4u);
    return 1;
}
