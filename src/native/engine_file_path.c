...
static int apply_search_path(const CPU *source, char *why, int whyn)
{
    ...
    if (!source) return fail(why, whyn, "no guest context for file-path setup");
    module = igcore_module();
    if (!module) return fail(why, whyn, "libIGCore is not linked");
...
static void x2_engine_file_path_register_override(void)
{
    x86_register_override("libIGCore.dll",
                          IGFILE_SET_SEARCH_PATH_FROM_REGISTRY,
