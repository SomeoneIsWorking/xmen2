...
int pe_map(const char *path, PeImage *out) {
  ...
  for (i = 0; i < (int)nsec; i++) {
    ...
    if (n)
      memcpy(guest_memory_pointer(base + va), f + raw, n);
  }
...
void pe_unmap(PeImage *img) {
  if (img->base)
    guest_memory_release(img->base, img->size);
  img->base = 0;
}
