                             ...
                             uint8_t join_players, uint8_t leave_players)
{
    ...
    for (player = 0; player < 4u; player++) {
        ...
        int active;
        if (!((join_players | leave_players) & bit)) continue;
        active = (uint8_t)thiscall_player(
    ...
    if (changed) {
        ...
        fprintf(stderr, "PLAYER-PARTICIPATION: retail reconcile; join=0x%02x "
                                   ...
                                   uint8_t leave_players)
{
    ...
    if (!cpu || !(join_players | leave_players)) return;
    base = exe_base();
    if (!base) return;
    manager = guest_call0(cpu, base + PARTICIPATION_SINGLETON_RVA);
    if (!manager || !RD32(manager)) return;
                                                 ...
                                                 uint8_t eligible_players)
{
    ...
    for (player = 0; player < 4u; player++) {
        uint8_t bit = (uint8_t)(1u << player);
        if (!(eligible_players & bit) && (uint8_t)thiscall_player(
                cpu, manager, PARTICIPATION_ACTIVE, player) != 0)
            leave_players |= bit;
    }
    if (leave_players) apply_to_manager(cpu, manager, 0u, leave_players);
}
