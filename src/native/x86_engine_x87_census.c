...
static const char *const kOpNames[X86P_X87_OPS] = {"FADD", "FSUB", "FMUL",
                                                   "FDIV"};
/* The control word's own field encodings, in its own order. */
static const char *const kPrecisionNames[4] = {
    "24-bit (single)", "reserved", "53-bit (double)", "64-bit (extended)"};
static const char *const kRoundingNames[4] = {"nearest-even", "down", "up",
                                              "toward zero"};
...
static int armed(void) {
  if (g_armed < 0) {
    ...
    if (g_armed) {
      lucent_log_info(
          "engine", "x87.census=1: every x87 arithmetic operation is counted, "
...
void x86_engine_x87_census_report(const char *tag) {
  ...
  for (i = 0; i < 4u; i++) {
    if (g_census.by_precision[i] != 0u) {
      lucent_log_info(
          "engine",
          "%sx87 census: precision control %s on %llu operation(s) (%.1f%%)",
  ...
  for (i = 0; i < 4u; i++) {
    if (g_census.by_rounding[i] != 0u) {
      lucent_log_info(
          "engine", "%sx87 census: rounding %s on %llu operation(s) (%.1f%%)",
          tag, kRoundingNames[i], (unsigned long long)g_census.by_rounding[i],
  ...
  for (i = 0; i < X86P_X87_OPS; i++) {
    ...
    if (g_census.ordinary_measured) {
      ...
      lucent_log_info(
          ...
          taken, ops ? 100.0 * (double)taken / (double)ops : 0.0,
      ...
      lucent_log_info("engine",
                      ...
                      tag, kOpNames[i], ops,
                      100.0 * (double)ops / (double)total, total);
    }
  }
}
