 ...
 * (x86p_jit_engine_set_boundary). The title call loop makes the same decision
 * around each bounded JIT run. Servicing a hand-back -- running the thunk or
 ...
 * return trampoline). `entry` is the current guest call's own entry
 * point, which is never handed back to itself. Shared by the title call loop
 ...
 * check (stop the instant the current guest call returns) as a
