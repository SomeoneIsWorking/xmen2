...
static void mouse_geometry(MouseGeometry *geometry)
{
    ...
    if (!g_window) {
        fprintf(stderr, "win32 pointer: cursor coordinates require an attached guest window\n");
        abort();
    }
...
static int32_t coordinate_add(int32_t coordinate, int64_t offset, const char *axis)
{
    int64_t result = (int64_t)coordinate + offset;

    if (result < INT32_MIN || result > INT32_MAX) {
        fprintf(stderr, "win32 pointer: %s cursor coordinate is outside the guest 32-bit range\n",
                axis);
        abort();
    }
...
static void map_point(float host_x, float host_y, int32_t *client_x, int32_t *client_y,
                      int32_t *screen_x, int32_t *screen_y)
{
    ...
    mouse_geometry(&geometry);
    if (!x2_win32_mouse_map_point(x, y, geometry.window_width, geometry.window_height,
                                  geometry.game_width, geometry.game_height,
                                  client_x, client_y)) {
        fprintf(stderr, "win32 pointer: cannot map (%d,%d) from window %ux%u to game %ux%u\n",
...
int x2_win32_pointer_get_cursor_pos(int32_t *x, int32_t *y)
{
    ...
    if (!x2_win32_mouse_map_point(
            coordinate_add((int32_t)global_x, -(int64_t)geometry.window_x, "horizontal"),
            coordinate_add((int32_t)global_y, -(int64_t)geometry.window_y, "vertical"),
            geometry.window_width, geometry.window_height,
            geometry.game_width, geometry.game_height, &client_x, &client_y)) {
        fprintf(stderr, "win32 pointer: cannot map physical cursor to game coordinates\n");
        abort();
    }
...
int x2_win32_pointer_set_cursor_pos(int32_t x, int32_t y)
{
    ...
    if (!x2_win32_mouse_unmap_point(
        ...
        fprintf(stderr, "win32 pointer: cannot map game cursor to window coordinates\n");
        abort();
    }
...
static uint32_t modifiers(void)
{
    ...
    if (state & SDL_KMOD_SHIFT) result |= X2_MK_SHIFT;
    if (state & SDL_KMOD_CTRL) result |= X2_MK_CONTROL;
    return result;
}
...
static uint32_t buttons(SDL_MouseButtonFlags state)
{
    ...
    if (state & SDL_BUTTON_LMASK) result |= X2_MK_LBUTTON;
    if (state & SDL_BUTTON_RMASK) result |= X2_MK_RBUTTON;
    if (state & SDL_BUTTON_MMASK) result |= X2_MK_MBUTTON;
    return result;
}
...
{
    if (queued) return;
    fprintf(stderr, "win32 pointer: ordered queue filled while posting %s; refusing to discard it\n",
            kind);
    abort();
}
                                      ...
                                      X2Win32Mouse *mouse, uint32_t hwnd)
{
    ...
    if (event->type == SDL_EVENT_MOUSE_MOTION) {
        map_point(event->motion.x, event->motion.y,
                  &client_x, &client_y, &screen_x, &screen_y);
        queued = x2_win32_mouse_motion(mouse, hwnd, client_x, client_y, screen_x, screen_y,
        ...
        if (event->type != SDL_EVENT_MOUSE_BUTTON_DOWN &&
            event->type != SDL_EVENT_MOUSE_BUTTON_UP) return;
        ...
        map_point(event->button.x, event->button.y,
                  &client_x, &client_y, &screen_x, &screen_y);
        queued = x2_win32_mouse_button(mouse, hwnd, button,
                                       event->type == SDL_EVENT_MOUSE_BUTTON_DOWN,
                                       client_x, client_y, screen_x, screen_y,
                                       (uint32_t)(event->button.timestamp / 1000000u),
                                       modifiers());
    }
                                      ...
                                      X2Win32Mouse *mouse, uint32_t hwnd)
{
    ...
    if (!pointer || !pointer->valid) return;
    ...
    if (queued && pointer->button_change >= 0)
        queued = x2_win32_mouse_button(mouse, hwnd, X2_WIN32_MOUSE_LEFT,
                                       pointer->button_change != 0,
                                       client_x, client_y, screen_x, screen_y,
