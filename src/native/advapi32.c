...
static void path_join(char *out, size_t cap, const char *parent,
                      const char *sub) {
  if (!sub || !*sub) {
...
void advapi32_store_save(void) {
  ...
  for (i = 0; i < MAX_VALUES; i++) {
    ...
    for (j = 0; j < g_val[i].len; j++)
      fprintf(f, "%02x", g_val[i].data[j]);
    fputc('\n', f);
  }
  fclose(f);
  g_dirty = 0;
...
static void reg_create(CPU *C, int ex) {
  ...
  if (disp)
    WR32(disp, existed ? 2u : 1u); /* OPENED : CREATED */
  ret_std(C, ERROR_SUCCESS, nargs);
}
...
void imp_ADVAPI32_RegEnumKeyExA(CPU *C) {
  ...
  for (i = 0; i < MAX_VALUES; i++) {
    ...
    p = g_val[i].path + n + 1;
    sep = strchr(p, '\\');
    snprintf(child, sizeof child, "%.*s", sep ? (int)(sep - p) : (int)strlen(p),
             p);
    ...
    for (j = 0; j < i; j++) {
      ...
      q = g_val[j].path + n + 1;
      qs = strchr(q, '\\');
      snprintf(other, sizeof other, "%.*s", qs ? (int)(qs - q) : (int)strlen(q),
               q);
      if (strcasecmp(other, child) == 0) {
        dup = 1;
        break;
      }
    }
      ...
      continue;
    {
      ...
      if (pcname && RD32(pcname) < len) {
        ret_std(C, ERROR_MORE_DATA, 8);
        return;
      }
      if (out)
        memcpy(guest_memory_pointer(out), child, len);
      if (pcname)
        WR32(pcname, len - 1u);
    }
...
void imp_ADVAPI32_RegEnumValueA(CPU *C) {
  ...
  for (i = 0; i < MAX_VALUES; i++) {
      ...
      continue;
    {
      ...
      if (out)
        memcpy(guest_memory_pointer(out), v->name, len);
      if (pcname)
        WR32(pcname, len - 1u);
    }
    if (ptype)
...
void advapi32_install(void) {
  ...
  x86_native_export("ADVAPI32.DLL", "RegCreateKeyA",
                    imp_ADVAPI32_RegCreateKeyA);
  x86_native_export("ADVAPI32.DLL", "RegCreateKeyExA",
                    imp_ADVAPI32_RegCreateKeyExA);
  x86_native_export("ADVAPI32.DLL", "RegQueryValueA",
                    imp_ADVAPI32_RegQueryValueA);
  x86_native_export("ADVAPI32.DLL", "RegQueryValueExA",
                    imp_ADVAPI32_RegQueryValueExA);
  x86_native_export("ADVAPI32.DLL", "RegSetValueExA",
                    imp_ADVAPI32_RegSetValueExA);
  x86_native_export("ADVAPI32.DLL", "RegEnumKeyExA",
                    imp_ADVAPI32_RegEnumKeyExA);
  x86_native_export("ADVAPI32.DLL", "RegEnumValueA",
                    imp_ADVAPI32_RegEnumValueA);
