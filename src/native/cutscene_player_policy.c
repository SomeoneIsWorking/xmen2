...
int x2_cutscene_player_inherits_context(int sequence_active,
                                        int owned_parent,
                                        int owned_event,
                                        int owned_payload)
{
    return sequence_active &&
           (owned_parent || owned_event || owned_payload);
}
...
X2CutscenePlayerResult x2_cutscene_player_finish(
    X2CutscenePlayerPolicy *policy, const X2CutscenePlayerOps *ops,
    void *context)
{
    ...
    if (!policy) return X2_CUTSCENE_PLAYER_ERROR;
    if (!operations_valid(ops)) return fail(policy, X2_CUTSCENE_PLAYER_ERROR);

    policy->requests++;
    active = ops->active_sequence(context, &sequence);
    if (active < 0) return fail(policy, X2_CUTSCENE_PLAYER_ERROR);
    if (!active) return X2_CUTSCENE_PLAYER_INACTIVE;

    policy->invocations++;
    if (ops->control_state(context, sequence) !=
        X2_CUTSCENE_CONTROL_LOCKED)
        return fail(policy, X2_CUTSCENE_PLAYER_ERROR);

    limit = policy->step_limit ? policy->step_limit
                               : X2_CUTSCENE_DEFAULT_STEP_LIMIT;
    for (steps = 0; steps < limit; steps++) {
        ...
        step = ops->step_owned_fiber(context, sequence, fiber,
                                     &conversation);
        switch (step) {
        ...
        case X2_CUTSCENE_FIBER_DETERMINISTIC_CONVERSATION:
            if (!ops->play_deterministic_conversation(
                    context, sequence, conversation))
                return fail(policy, X2_CUTSCENE_PLAYER_ERROR);
