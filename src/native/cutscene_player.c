...
static void own(uint32_t context) {
  if (!context || owned_index(context) >= 0)
    return;
  if (g_player.owned_count >= OWNED_CONTEXT_LIMIT)
    return;
...
x2_cutscene_player_register_overrides(void) {
  x86_register_override("XMen2.exe", 0x00469130u, x2_override_00469130);
  x86_register_override("XMen2.exe", 0x004a00d0u, x2_override_004a00d0);
  x86_register_override("XMen2.exe", 0x004d7c10u, x2_override_004d7c10);
  x86_register_override("XMen2.exe", 0x004d8700u, x2_override_004d8700);
}
