...
{
    if (g_start_real == 0.0) g_start_real = real_now_s();
...
{
    return (uint64_t)(guest_clock_now_s() * 1e9);
}
...
int guest_clock_skip_idle_to(double deadline)
{
    ...
    g_idle_calls++;
    if (!guest_clock_unbounded()) return 0;
    ...
    g_skips++;
    g_skipped_s += dt;
    if (dt > g_largest_skip) g_largest_skip = dt;
    return 1;
}
...
void guest_clock_report(void)
{
    ...
    fprintf(stderr,
        ...
        g_skips, g_idle_calls, g_skipped_s, g_largest_skip,
        g_refused_backwards, real, real + g_skew);
}
