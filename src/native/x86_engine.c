...
int x2_engine_call(uint32_t addr, CPU *C) {
  ...
  /* The JIT's inline dispatch handler (x86_engine_jit_dispatch) is called by
     x86port with only the engine's user pointer, so this frame's identity is
     threaded to it here. Intrusive stack on the C stack: unlimited nesting,
     restored explicitly after a longjmp unwinds the nested pushes. */
  EngineCallCtx ctx_slot;
  x86_engine_call_ctx_push(&ctx_slot, C, addr, return_to, entry_esp);
  ...
  for (;;) {
    ...
    if (cpu.eip != entry && x86_setjmp3_thunk(cpu.eip)) {
      ...
      if (rc) {
        ...
        x86_engine_call_ctx_restore(&ctx_slot);
