...
static const struct {
  SDL_Scancode sdl;
  unsigned char dik;
} DIK_MAP[] = {{SDL_SCANCODE_ESCAPE, 0x01},
               {SDL_SCANCODE_1, 0x02},
               {SDL_SCANCODE_2, 0x03},
               {SDL_SCANCODE_3, 0x04},
               {SDL_SCANCODE_4, 0x05},
...
unsigned long dinput_system_blind_reads(void) { return g_blind_reads; }
...
unsigned char dinput_system_dik(int scancode) {
  ...
  for (i = 0; i < DIK_MAP_N; i++)
    if (DIK_MAP[i].sdl == scancode)
      return DIK_MAP[i].dik;
#else
  (void)scancode;
#endif
  return 0;
...
const char *dinput_system_dik_name(unsigned char dik) {
  ...
  int i;
  for (i = 0; i < DIK_MAP_N; i++)
    if (DIK_MAP[i].dik == dik)
      return SDL_GetScancodeName(DIK_MAP[i].sdl);
#else
  (void)dik;
#endif
  return NULL;
...
void dinput_system_keyboard_state(uint32_t out, uint32_t size) {
...
#ifdef X2_WITH_SDL
  {
    ...
    for (i = 0; i < DIK_MAP_N; i++) {
      if (DIK_MAP[i].sdl >= nkeys || !keys[DIK_MAP[i].sdl])
        continue;
      if ((uint32_t)DIK_MAP[i].dik >= size)
        continue;
...
void dinput_system_mouse_state(uint32_t out, uint32_t size) {
...
#ifdef X2_WITH_SDL
  {
    ...
    if (size >= 4u)
      WR32(out, (uint32_t)(int32_t)dx);
    if (size >= 8u)
      WR32(out + 4u, (uint32_t)(int32_t)dy);
...
const unsigned char *dinput_guid_of(int kind) {
  return kind == DINPUT_DEV_KEYBOARD ? GUID_SYS_KEYBOARD
         : kind == DINPUT_DEV_MOUSE  ? GUID_SYS_MOUSE
                                     : NULL;
}
