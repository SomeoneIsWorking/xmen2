...
void imp_USER32_LoadIconA(CPU *C) {
  static uint32_t t = 0x00E10000u;
  ret_std(C, ++t, 2);
}
void imp_USER32_LoadCursorA(CPU *C) {
  static uint32_t t = 0x00E20000u;
  ret_std(C, ++t, 2);
}
...
static int gwl_index(int32_t idx) {
  ...
  case -20:
    return 3;
  case -6:
    return 4; /* GWL_HINSTANCE */
  case -8:
    return 5; /* GWL_HWNDPARENT */
  case -12:
    return 6; /* GWL_ID */
  default:
    return -1;
...
void imp_USER32_SetWindowPos(CPU *C) {
  ...
  if (g_win && hwnd_is_main(A(0)) && !x2_window_settings_owns_geometry()) {
    ...
    if (!(flags & SWP_NOSIZE))
      SDL_SetWindowSize(g_win, (int)A(4), (int)A(5));
  }
  ret_std(C, 1, 7);
}
...
void imp_USER32_GetSystemMetrics(CPU *C) {
  ...
  SDL_DisplayID d = SDL_GetPrimaryDisplay();
  if (d)
    dm = SDL_GetCurrentDisplayMode(d);
  switch (idx) {
  case 0:
  case 16:
    v = dm ? (uint32_t)dm->w : 800u;
    break;
  case 1:
  case 17:
    v = dm ? (uint32_t)dm->h : 600u;
    break;
  case 4:
    v = 0;
    break; /* SM_CYCAPTION -- borderless */
  case 32:
  case 33:
    v = 0;
...
void imp_USER32_PtInRect(CPU *C) {
  ...
  l = (int32_t)RD32(r);
  t = (int32_t)RD32(r + 4u);
  ri = (int32_t)RD32(r + 8u);
  b = (int32_t)RD32(r + 12u);
...
void imp_USER32_MessageBoxA(CPU *C) {
   ...
   */
  {
    ...
    ret_std(C,
            (uint32_t)win32_sdl_dialog(cap ? cap : "X-Men Legends II",
                                       text ? text : "(no text)", lbl, ids, n,
                                       fallback),
            4);
  }
}
...
void imp_USER32_ClipCursor(CPU *C) {
  ...
  if (g_win_live)
    SDL_SetWindowMouseGrab(g_win, A(0) != 0u);
  ret_std(C, 1, 1);
}
...
static uint32_t data_alloc(uint32_t v) {
  uint32_t a;
  if (!g_data_arena || g_data_used + 4u > g_data_size)
    return 0;
...
uint32_t x86_native_data_export(const char *mod, const char *sym) {
  ...
  if (strcasecmp(mod, "MSVCRT.dll") == 0 ||
    ...
    if (strcmp(sym, "_acmdln") == 0) {
      ...
      if (s2)
        *(volatile uint8_t *)guest_memory_pointer(s2) = 0;
...
static const struct {
  unsigned char sc, vk;
} VK_OF_SCANCODE_EXT[] = {{0x9C, 0x0D}, /* numpad enter */
...
void imp_USER32_MapVirtualKeyA(CPU *C) {
  ...
  case MAPVK_VSC_TO_VK_EX:
    if (code < 128u) {
      ret_std(C, VK_OF_SCANCODE[code], 2);
      return;
    }
    for (i = 0; i < sizeof VK_OF_SCANCODE_EXT / sizeof VK_OF_SCANCODE_EXT[0];
         i++)
