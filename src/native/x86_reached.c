...
static struct {
  uint32_t ep, base, seq, n;
} g_reached[REACHED_SLOTS];
...
static unsigned reached_slot(uint32_t ep, uint32_t base) {
  ...
  for (;;) {
    ...
    if (g_reached[i].ep == 0)
      return i;
    if (g_reached[i].ep == ep && g_reached[i].base == base)
      return i;
    h++;
  }
}
...
void x86_reached_arm(const char *why) {
  ...
  x86_reached_armed = 1;
  fprintf(stderr,
          "[REACHED] armed (%s). Everything entered BEFORE this "
...
static void reached_selftest(void) {
  ...
  fprintf(stderr,
          "[REACHED] selftest: inserted -> %s; never-inserted -> %s; "
          "order %u<%u -> %s; counts 2/1 -> %s; same ep in two "
          "modules kept apart -> %s\n",
          ok_pos ? "REACHED (correct)" : "NEVER (WRONG)",
          ok_neg ? "NEVER (correct)" : "REACHED (WRONG)", reached_seq(a, B1),
          reached_seq(b, B1), ok_ord ? "correct" : "WRONG",
...
int x86_reached_query(uint32_t linked_ep, const char *module,
  ...
  for (m = x86_modules(); m; m = m->next) {
    uint32_t s;
    if (module && (!m->name || strcmp(m->name, module)))
      continue;
    ...
    found = 1;
    if (count)
      *count += reached_count(linked_ep, *m->base);
    if (seq && (!*seq || s < *seq))
      *seq = s;
  }
  return found;
...
void x86_reached_report(void) {
  ...
  fprintf(stderr,
          "[REACHED] %u distinct (entry point, module) pair(s) were "
          "entered%s%s.\n",
          g_reached_n,
          g_reached_why ? ", armed by "
                        : " (armed from the first "
                          "instruction)",
  ...
  for (p = strtok_r(buf, ",", &save); p; p = strtok_r(NULL, ",", &save)) {
    ...
    for (m = x86_modules(); m; m = m->next) {
      ...
      if (ep < m->preferred || ep - m->preferred >= m->size)
        continue;
      ...
      if (seq)
        fprintf(stderr, "[REACHED]   0x%08x  REACHED  #%-6u x%-6u %s\n", ep,
                seq, reached_count(ep, *m->base), m->name);
      else
    ...
    if (!nmod)
      fprintf(stderr,
              "[REACHED]   0x%08x  -- NO registered module "
...
void x86_reached_route(int fd, const char *query) {
  ...
  if (!control_query_arg(query, "ep", ep_text, sizeof ep_text)) {
    snprintf(body, sizeof body,
             ...
             "disassembly shows.\n",
             armed ? "ARMED and recording"
                   : "NOT armed; arm it with "
                     "/reached?arm=1");
