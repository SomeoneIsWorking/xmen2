...
static void capture_row(uint32_t page, unsigned resident,
                        ResidentRow *row)
{
    memcpy(row->text,
...
{
    memset(guest_memory_pointer(page + PAGE_TEXT + resident * ROW_BYTES),
           0, ROW_BYTES);
    memset(guest_memory_pointer(page + PAGE_PRIMARY + resident * ROW_BYTES),
           0, ROW_BYTES);
    memset(guest_memory_pointer(page + PAGE_SECONDARY + resident * ROW_BYTES),
           0, ROW_BYTES);
}
...
static void update_selected_leaf(void)
{
    ...
    g_selected_leaf[0] = 0;
    if (!g_active || g_window.selected >= g_plan.count) return;
...
static void refresh_projection(void)
{
    ...
    for (resident = 0u; resident < X2_LOAD_GAME_VISIBLE_ROWS; resident++) {
        ...
        if (!x2_load_game_menu_window_entry(&g_plan, &g_window, resident,
                                             &entry)) {
            g_active = 0;
            return;
        }
        ...
        if (row->primary_bit) primary_bits |= 1u << resident;
        if (row->secondary_bit) secondary_bits |= 1u << resident;
        if (row->selectable_bit) selectable_bits |= 1u << resident;
    }
...
static uint16_t manual_present_mask(uint32_t manager)
{
    ...
    for (slot = 0u; slot < X2_LOAD_GAME_MANUAL_SLOTS; slot++) {
        uint32_t metadata = manager + MANAGER_METADATA
                          + slot * METADATA_STRIDE;
...
static void make_autosave_row(ResidentRow *row)
{
    ...
    while (length + 1u < sizeof description
        ...
        description[length] = (char)RD8(g_autosave_metadata
                                        + (uint32_t)length);
        length++;
    }
...
static void activate_projection(const CPU *source, uint32_t manager)
{
    ...
    g_active = 0;
    g_selected_leaf[0] = 0;
    if (!exe_base() || RD32(manager + MANAGER_MODE) != SAVE_MODE_LOAD) return;
    g_ui = RD32(g_exe + UI_SINGLETON);
    g_page = dialog_page(g_ui);
    if (!g_ui || !g_page) return;
    ...
    if (manual_count && retail_count != manual_count) return;
...
{
    return g_active && ui == g_ui && dialog_page(ui) == g_page
        && g_manager && RD32(g_manager + MANAGER_MODE) == SAVE_MODE_LOAD;
}

static int poll_navigation_delta(const CPU *source, uint32_t ui,
                                 int *delta)
{
    CPU call = *source;
    uint32_t input;
    ...
    call.esp -= 4u; WR32(call.esp, sample);
    call.ecx = input;
...
static void x2_override_005e9d30(CPU *C)
{
    ...
    if (x2_load_game_menu_window_select(&g_plan, &g_window,
                                        resident_focus))
        update_selected_leaf();
...
size_t x2_load_game_menu_runtime_report(char *out, size_t capacity)
{
    ...
    long selected = g_active && g_window.selected < g_plan.count
        ? (long)g_window.selected : -1l;
    size_t resident = g_active
        ? x2_load_game_menu_window_count(&g_plan, &g_window) : 0u;
    ...
    written = snprintf(out, capacity,
        ...
        g_active ? g_plan.count : 0u, resident,
        g_active ? g_window.first : 0u, selected,
        g_selected_leaf[0] ? g_selected_leaf : "none",
...
static void x2_load_game_menu_register(void)
{
    x86_register_override("XMen2.exe", 0x004b0d20u,
                          x2_override_004b0d20);
    x86_register_override("XMen2.exe", 0x005e9d30u,
                          x2_override_005e9d30);
    x86_register_override("XMen2.exe", 0x0049f010u,
                          x2_override_0049f010);
}
