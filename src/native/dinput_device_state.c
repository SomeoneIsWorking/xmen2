...
static int joystick_active(uint32_t out, uint32_t bytes, int32_t lo,
  ...
  for (i = 0; i < 6u; i++) {
    ...
    if (value < mid - threshold || value > mid + threshold)
      return 1;
  }
  if (RD32(out + 32u) != 0xFFFFFFFFu)
    return 1;
  for (i = 48u; i < bytes && i < 80u; i++)
    if (*((unsigned char *)guest_memory_pointer(out + i)) & 0x80u)
      return 1;
  return 0;
}

static void record_state(const DInputDevice *device, uint32_t out,
                         uint32_t bytes) {
