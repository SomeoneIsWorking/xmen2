...
def load():
    """Parse the project's re-frontier.md into {id: Entry}, preserving area order."""
    entries = {}
    order = []
    area = "misc"
    cur = None
    # FAIL LOUDLY on a missing roadmap. Reading a nonexistent file used to be indistinguishable from
    # reading a healthy one: `load()` returned zero entries, and `check` then reported
    # "OK: no unknown deps, no cycles, every re-verified step cites evidence" — a green result over
    # NOTHING. That is the worst failure mode a validator can have, and it is easy to hit because the
    # default ROADMAP assumes the tool lives at <repo>/tools/, which is false when it runs from a
    # global skill dir: it resolved to ~/.claude/skills/docs/re-frontier.md, a path that never exists.
    # Observed in the wild — a project cited `check` for a whole session while it validated nothing.
    if not os.path.isfile(ROADMAP):
        sys.exit(f"re-frontier: no roadmap at {ROADMAP}\n"
                 f"  Set RE_FRONTIER_ROADMAP to the project's frontier file, e.g.\n"
                 f"    RE_FRONTIER_ROADMAP=docs/re-frontier.md {sys.argv[0]} tree\n"
                 f"  (the default only works when this tool lives at <repo>/tools/).")
...
def main():
    ...
    args = p.parse_args()
    # scaffold BOOTSTRAPS the roadmap, so it must not require the roadmap to
    # already exist -- load() exits when the file is missing, which made
    # scaffold impossible to run on a project that had never used the tool.
    entries, order = ({}, []) if args.cmd == "scaffold" else load()
    fn = {
