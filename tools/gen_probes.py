...
one side simply has no records. src/native/overrides.json carries the same
warning because the Xbox side lost two call sites to it. gen_overrides.py owns
the isolate files and reads this manifest for its half, so there is exactly one
writer.
...
def emit_wraps(probes):
    ...
    for i, p in enumerate(probes):
        ...
        L.append("void __wrap_%s(CPU *C)\n{\n"
                 "    oracle_probe_call(&g_probes[%d], __real_%s, C);\n}\n"
                 % (sym, i, sym))
...
def isolate_eps():
    """The (module, ep) pairs gen_overrides.py must isolate for probes.

    Imported by gen_overrides so there is ONE writer of the isolate files."""
    try:
