 ...
 *       scratch/analysis/libIGGfx.vtab.json --class %(cls)s \\
...
def main(argv):
    ...
    ap.add_argument(
        "--list", action="store_true", help="print every slot with its name and verdict"
    )
    ...
    if a.cls not in vt:
        sys.exit(
            "device_slots: %s has no vtable in %s. Known: %s"
    ...
    print(
        "device_slots: %s -- %d of %d slots reach %s; %d unscanned"
        % (a.cls, len(hits), len(slots), a.fields, len(unscanned)),
        file=sys.stderr,
    )
    if unscanned:
        print(
            "  UNSCANNED slots (absent from the list, so INHERITED): %s"
            % ", ".join(str(i) for i in unscanned),
            file=sys.stderr,
        )
        ...
        with open(a.header, "w") as f:
            f.write(
                ...
                % dict(
                    ...
                    fieldlist="/".join("0x%x" % x for x in sorted(fields)),
                    body=body,
                )
            )
