...
the ported engine path hands it, and that those lights are nearly black; the
    ...
    with open(path, "r", errors="replace") as fh:
        for line in fh:
            ...
            if kind == "SETLIGHT":
                ...
                key = (fields.get("diffuse"), fields.get("ambient"), fields.get("emissive"))
...
def refuse_unless_usable(side, d):
    ...
    if d["total"] == 0:
        sys.exit(
            "lightlog_diff: the %s log %s is EMPTY -- the run wrote no "
            "lines at all. Compared NOTHING." % (side, d["path"])
        )
    if not d["lights"]:
        sys.exit(
            "lightlog_diff: the %s log %s has %d line(s) but NO SETLIGHT "
            "-- either the run never reached a lit scene or the logger is "
            "not wired in. Compared NOTHING." % (side, d["path"], d["total"])
        )
    if d["unparsed"]:
        sys.exit(
            "lightlog_diff: the %s log %s has %d line(s) this tool could "
...
def fmt_values(entry, limit=4):
    ...
    for key, n in items[:limit]:
        if key == ("NULL-POINTER",):
            ...
            out.append(
                "    x%-6d type=%s diffuse=%s ambient=%s range=%s "
                "atten=%s" % (n, t, dif, amb, rng, att)
            )
    if len(items) > limit:
        out.append("    ... and %d more distinct value(s), not shown" % (len(items) - limit))
    return out
...
def report(control, port):
    ...
    print(
        "LIGHT INDICES: %d in the control, %d in the port, %d in either"
    ...
    for idx in idxs:
        ...
        def enabled(e):
            if e is None:
                return "never called"
            return "on x%d, off x%d, last=%s" % (e["on"], e["off"], "ON" if e["last"] else "off")

        print("index %d" % idx)
        print(
            "  control: %s SetLight call(s); LightEnable %s"
        ...
        if cl:
            for line in fmt_values(cl):
                print(line)
        print(
            "  port:    %s SetLight call(s); LightEnable %s"
        ...
        if c_on != p_on:
            diffs.append(
                "index %d was ever ENABLED in the %s but NEVER in the "
                "%s" % (idx, "control" if c_on else "port", "port" if c_on else "control")
            )
        if cl and pl:
            cv = set(cl["values"])
            pv = set(pl["values"])
            if not (cv & pv):
                diffs.append(
                    "index %d carried NO value in common: control %s, "
                    "port %s" % (idx, sorted(cv)[0], sorted(pv)[0])
                )
        elif cl and not pl:
            diffs.append("index %d was SET in the control and never in the port" % idx)
        elif pl and not cl:
            diffs.append("index %d was SET in the port and never in the control" % idx)
        print()
    ...
    for side, d in (("control", control), ("port", port)):
        if d["states"]:
            print(
                "  %-8s %s"
                % (side, ", ".join("%s x%d" % (k, v) for k, v in sorted(d["states"].items())))
            )
        else:
            print("  %-8s no LIGHTING or AMBIENT state was ever set" % side)
    print()
    print(
        "MATERIALS: %d distinct in the control, %d in the port"
        % (len(control["materials"]), len(port["materials"]))
    )
    print()

    print("SUMMARY -- %d difference(s) that do not depend on run timing" % len(diffs))
    for d in diffs:
        print("  * %s" % d)
    if not diffs:
...
SELF_B = SELF_A.replace(
    "diffuse=1.0000,1.0000,1.0000,1.0000", "diffuse=0.0000,0.0000,0.0000,1.0000"
).replace("LIGHTENABLE t=4 idx=4 on=1", "LIGHTENABLE t=4 idx=4 on=0")
...
def selftest():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for side, d in (("a", da), ("b", db)):
            if d["unparsed"]:
                print(
                    "SELFTEST FAIL: log %s had %d unparsed line(s): %s"
        ...
        if da["total"] != 8:
            print(
                "SELFTEST FAIL: known non-light proxy records were not read "
                "from the same stream (got %d total line(s), want 8)" % da["total"]
            )
            ok = False
        ...
        if len(diffs) != want:
            print(
                "SELFTEST FAIL: a black-and-disabled light against a white "
                "enabled one must produce %d difference(s); produced %d" % (want, len(diffs))
            )
            ok = False
        ...
        if same:
            print(
                "SELFTEST FAIL: a log compared against ITSELF produced %d difference(s)" % len(same)
            )
            ok = False
        ...
        if os.WEXITSTATUS(status) == 0:
            print(
                "SELFTEST FAIL: a log with no SETLIGHT was ACCEPTED; it must "
    ...
    print(
        "SELFTEST: %s"
        % (
            "PASS -- the comparison shows both answers and refuses an unusable log"
            if ok
            else "FAIL"
        )
    )
