...
NATIVE_BODY = """
/* ---- native (POSIX) runtime -------------------------------------------
 *
 * The Wine-hosted DLL build and this one share every emitted function body and
 * NOTHING else. There the recompiled code is entered by real Windows code, so
 * the guest stack is the host thread's stack and the two have to be kept apart
 * by hand (C080). Here WE are the caller: the guest stack is memory we
 * allocate and the C frames are on the ordinary C stack, so the two never
 * touch and none of that machinery exists.
 *
 * Guest addresses stay host addresses, as they do in the DLL build -- every PE
 * base in this game is below 4 GB, so the image maps at its preferred base
 * even in a 64-bit process. x86_native_map() is what has to prove that; if the
 * mapping lands anywhere else it fails rather than running against memory that
 * is not the image.
 */
uint32_t g_imgbase = 0x10000000U;
uint32_t g_image_lo, g_image_hi;
...
/* No original image is mapped alongside us and no Windows loader resolved
   anything, so there is no honest fallback: every one of these is a stop that
   names what is missing. A native build that quietly ran original machine code
   would not be a native build. */
void x86_note_fallback(uint32_t target)
{
...
void x86_missing_import(const char *mod, const char *sym)
{
    fprintf(stderr, "x86_missing_import: %s!%s is not implemented natively.\\n"
                    "  This is the native import surface, and it is the work "
                    "that replaces Wine.\\n", mod, sym);
    abort();
}
...
void x86_return_to(CPU *C, uint32_t target)
{
    ...
    for (i = 0; i < g_fn_count; i++)
        if (g_fns[i].ep == target) { g_fns[i].fn(C); return; }
    fprintf(stderr, "x86_return_to: 0x%08x is not a function entry\\n", target);
    abort();
}
...
/* Call a recompiled function by guest entry point. Returns 0 if there is no
   such body -- the caller is expected to say so rather than treat a miss as a
   zero result. */
int x86_native_call(uint32_t ep, CPU *C)
{
    int i;
    for (i = 0; i < g_fn_count; i++)
        if (g_fns[i].ep == ep) { g_fns[i].fn(C); return 1; }
    return 0;
}
...
const char *x86_native_name(uint32_t ep)
{
    int i;
    for (i = 0; i < g_fn_count; i++)
        if (g_fns[i].ep == ep) return g_fns[i].name;
    return NULL;
}
"""
...
def cmd_native(argv):
    """Emit the NATIVE (non-Windows) runtime: function table, POSIX runtime,
    and an aborting stub for every import.

    Separate from `runtime` because that one now carries the private runtime
    stack, which is a PE-hosting fix (C080) with no meaning here, and includes
    <windows.h>. Sharing them would drag Windows into the native build to
    solve a problem the native build does not have.
    """
    d = load(argv[0])
    out = argv[1]
    fns = [fn for fn in d["functions"] if translate(fn)[0] is not None]
