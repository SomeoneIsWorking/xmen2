...
def cmd_emit(argv):
    ...
    lines = [PROLOGUE % (d["program"], translator_fingerprint(),
                         base_sym, base_sym)]
...
def translator_fingerprint():
    """A content hash of the translator itself, stamped into everything it
    emits.

    The generated C is gitignored, so nothing tracked ever shows that it has
    fallen behind the translator that produces it -- and a stale module builds,
    links, runs and is WRONG in whatever way the fix that it missed was about
    to correct. That is not hypothetical: libIGSg was last emitted before both
    a tail-call ABI fix (209 sites in that module alone) and a reversed-x87
    fix, and the port drew warped characters for days because of it. It came
    right the moment the module was re-emitted for an unrelated reason, which
    is the worst possible way to find out.

    A CONTENT hash and not a git hash: an uncommitted edit to recomp.py changes
    what it emits just as much as a commit does.
    """
    import hashlib
    h = hashlib.sha256()
...
# The stamp line every emitted chunk carries. tools/check_emitted.py compares
# it against the translator as it is NOW and refuses a stale tree.
STAMP = "/* recomp-fingerprint: %s */"
