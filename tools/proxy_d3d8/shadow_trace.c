...
static const uint32_t SNAPSHOT_STATES[] = {D3DRS_ZENABLE,
                                           D3DRS_ZWRITEENABLE,
...
static void write_matrix(ShadowTrace *trace, const char *event, uint32_t which,
                         const float matrix[16]) {
  unsigned i;
  fprintf(trace->out, "{\"event\":\"%s\",\"frame\":%u,\"which\":%lu,\"m\":[",
...
static void write_snapshot(ShadowTrace *trace) {
  ...
  if (event_slot(trace))
    fprintf(trace->out,
            ...
            : trace->render_target_is_default ? "default"
                                              : "explicit",
            !trace->depth_stencil_known       ? "unknown"
            : trace->depth_stencil_is_default ? "default"
                                              : "explicit",
  ...
  for (stage = 0; stage < SHADOW_TRACE_STAGES; stage++) {
    if (event_slot(trace))
      fprintf(trace->out,
              ...
              trace->frame, stage, (unsigned long long)trace->texture[stage]);
...
static void begin_capture(ShadowTrace *trace, int detailed) {
  ...
  fprintf(trace->out,
          ...
          "\"max_events\":%u}\n",
          trace->frame, detailed, trace->expected_detailed, trace->max_events);
  write_snapshot(trace);
}

static void end_capture(ShadowTrace *trace) {
  fprintf(trace->out,
          ...
          "\"update_texture\":%u,\"default_render_target\":%u,"
          "\"default_depth_stencil\":%u}}\n",
          trace->frame, trace->capture_detailed, trace->draws,
          trace->indexed_draws, trace->events, trace->dropped_events,
          trace->resource_events, trace->dropped_resource_events,
...
void shadow_trace_default_targets(ShadowTrace *trace, uintptr_t target,
  ...
  shadow_trace_resource(trace, SHADOW_RESOURCE_DEFAULT_RENDER_TARGET, target, 0,
                        0, 0, 0, 0, 0, 0, target_result);
...
void shadow_trace_init(ShadowTrace *trace, FILE *out, int expected_detailed,
  ...
  trace->enabled =
      out != NULL && (expected_detailed == 0 || expected_detailed == 1);
...
void shadow_trace_present(ShadowTrace *trace, int detailed_shadow,
  ...
  begin_capture(trace, detailed_shadow);
}
...
void shadow_trace_resource(ShadowTrace *trace, ShadowResourceKind kind,
  ...
  if (!trace->enabled || kind >= SHADOW_RESOURCE_COUNT)
    return;
  trace->total_resources[kind]++;
  if (trace->capturing)
    trace->frame_resources[kind]++;
  if (trace->capturing) {
    if (!event_slot(trace))
      return;
  } else {
...
void shadow_trace_set_render_target(ShadowTrace *trace, uintptr_t target,
  ...
  if (!trace->enabled || !event_slot(trace))
    return;
  fprintf(trace->out,
...
void shadow_trace_set_transform(ShadowTrace *trace, uint32_t which,
                                const float matrix[16]) {
  if (which >= SHADOW_TRACE_TRANSFORMS || !matrix)
    return;
...
void shadow_trace_set_render_state(ShadowTrace *trace, uint32_t which,
                                   uint32_t value) {
  if (which >= SHADOW_TRACE_RENDER_STATES)
    return;
...
void shadow_trace_set_stage_state(ShadowTrace *trace, uint32_t stage,
  ...
  if (stage >= SHADOW_TRACE_STAGES || which >= SHADOW_TRACE_STAGE_STATES)
    return;
...
void shadow_trace_set_pixel_shader(ShadowTrace *trace, uint32_t handle) {
  ...
  fprintf(trace->out,
          ...
          "\"handle\":%lu}",
          trace->frame, (unsigned long)handle);
  flush_line(trace);
}
...
void shadow_trace_draw(ShadowTrace *trace, int indexed, uint32_t primitive,
  ...
  fprintf(
      ...
      "\"render_target_source\":\"%s\","
      "\"depth_stencil_source\":\"%s\"}",
      trace->frame, trace->draws, indexed ? 1 : 0, (unsigned long)primitive,
      (unsigned long)primitive_count, (unsigned long long)trace->render_target,
      (unsigned long long)trace->depth_stencil,
      (unsigned long)trace->pixel_shader, (unsigned long long)trace->texture[0],
      ...
      trace->render_state_set[D3DRS_STENCILENABLE] ? "explicit" : "default",
      trace->render_state_set[D3DRS_COLORWRITEENABLE] ? "explicit" : "default",
      !trace->render_target_known       ? "unknown"
      : trace->render_target_is_default ? "default"
                                        : "explicit",
      !trace->depth_stencil_known       ? "unknown"
...
void shadow_trace_clear(ShadowTrace *trace, uint32_t rect_count,
  ...
  if (!rects) {
    ...
    for (i = 0; i < rect_count; i++) {
      if (i)
        fputc(',', trace->out);
      fprintf(trace->out, "[%ld,%ld,%ld,%ld]", (long)rects[i * 4u],
              (long)rects[i * 4u + 1u], (long)rects[i * 4u + 2u],
              (long)rects[i * 4u + 3u]);
    }
    fputc(']', trace->out);
  }
  fprintf(trace->out,
          ",\"flags\":%lu,\"color\":%lu,\"depth\":", (unsigned long)flags,
          (unsigned long)color);
  write_float_json(trace->out, depth);
  fprintf(trace->out,
          ",\"stencil\":%lu,\"render_target\":\"0x%llx\","
          "\"depth_stencil\":\"0x%llx\",\"result\":%ld}",
          (unsigned long)stencil, (unsigned long long)trace->render_target,
