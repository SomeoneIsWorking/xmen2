...
The shadow extension is mechanism-verified but has not yet produced a stock capture. `test_shadow_trace` drives the shipping trace core through an ordinary frame (`path=none`) and independent planar, projective and self answers, plus a combined frame using a render-target texture, texture transform, stage state, depth bias, stencil, colour-write mask, Clear and a pixel shader. The ordinary-frame control proves untouched render state is reported from D3D8 defaults (including colour-write `0xf` and stencil masks `0xffffffff`) and that default render/depth targets are named rather than emitted as zero-initialized state. Separate cases prove expected/actual DetailedShadow mismatch, event overflow and incomplete texture-vtable coverage all refuse. `shadow_trace_compare.py --selftest` proves a planar-off/detailed-on answer and four refusal paths. `gen_probes.py --check` resolves and validates the exact prologues of `igPlanarShadowShader::shade`, `igProjectiveShadowShader::shade`, and `igCommonTraverseSelfShadowShader`; the stock proxy DLL compiles and links with those generated hooks. The self discriminator is the top-level traversal rather than `dispatchCasters`, because a reused depth map need not resubmit casters in every selected frame. Its cdecl frame is an explicit, hashed eight bytes; no process-specific object pointer is added to comparable fields merely to make the hook work. This does not yet prove which path the retail game selects.
...
There is no visible retail Detailed Shadows control: the setting inventory proved it is hidden backing state. The control therefore uses two independent proxy inputs. At `Direct3DCreate8`, after the retail settings loader and before D3D8 resource creation, `X2_SHADOW_FORCE` makes the proxy validate the exact executable store instruction and change only the live `XMen2.exe+0x668d40` byte; it immediately reads the byte back and records original, forced and observed values. `X2_SHADOW_EXPECT` is not the force value's alias: it remains the independent assertion checked again at F9, and a deliberate force/expect mismatch refuses. This is a bounded live-byte intervention, not a claim that the untouched UI exposes the setting, and it never writes the configured Wine prefix or registry. Reach the same gameplay scene in each run and press F9 once. F9 selects exactly the next D3D8 frame; resource creation is recorded in a bounded catalogue before selection so a render-target texture allocated during level load is not lost.
...
- The DetailedShadow address is build-specific. The proxy validates the executable instruction that writes it and records UNKNOWN on a different executable rather than reading the same RVA blindly.
