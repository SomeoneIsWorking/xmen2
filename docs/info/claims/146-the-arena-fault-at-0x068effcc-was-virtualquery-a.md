...
X2_VERBOSE=1: 'VirtualQuery(0x068ec000) -> COMMIT', then 'VirtualFree(0x068ec000, 16384, type 0x4000)', then the same query answering COMMIT again -- and the fault address 0x068effcc is inside that decommitted 16 KB. Earlier allocator hypotheses did not explain this disagreement. After the two calls were made to share one record of committed ranges, the run goes past igArenaSystemMalloc entirely and reaches the intro movie, stopping on WINMM.dll!timeSetEvent.
