...
FALSIFIED by its own stated falsifier, on the first run of the parse it asked for. The FuncInfo at 0x6c3d58 IS real -- magic 0x19930520, so the C++ EH half holds -- but it has nTryBlocks = 0 and a NULL TryBlockMap, so this function has no catch handlers at all and 0x005fb270 cannot be one. Checked the UnwindMap too (maxState 4 at 0x006c3d38): its four actions are 0x00679110, 0x00679118, 0x00679120 and 0x00679128, all small thunks beside the handler stub, and 0x005fb270 is not among them. So 0x005fb270 is neither a catch funclet nor a destructor funclet. WHAT SURVIVES: FUN_005fac10 genuinely is an MSVC C++ EH function (PUSH -1; PUSH 0x679141; MOV EAX,FS:[0]; and 0x679141 is MOV EAX,<FuncInfo>; JMP __CxxFrameHandler via MSVCR71), and seeding addresses inside it genuinely does carve it apart -- issue #21 stands on its own evidence. WHAT THIS REDIRECTS TO: with the EH tables ruled out, the simpler reading is better supported -- the exception-frame prologue and its MOV FS:[0],ESP are mistranslated, state is wrong by the time an indirect call is reached, and 0x005fb270 is a GARBAGE target that merely happens to land inside the same function. A consequence, not a cause. The enumerate-the-funclets tool this claim proposed would not have helped. Read what recomp.py emits for the exception-frame prologue first.
