...
TRANSLATOR HALF FIXED AND VERIFIED BY UNIT TEST, database half still open. The defect was that _has_injmp gated on the exporter's 'ind' flag, which Ghidra does not set on JMP dword ptr [reg*4 + <table>] -- that form arrives with neither 'ind' nor 'flow' -- so the local dispatcher was never generated for any switch in the image and every one fell through to the global dispatcher. tests/test_recomp.py drives recomp.translate() directly: 4 of 6 cases fail on the old predicate and name the exact emitted line, DISPATCH(C, RD32((uint32_t)((C->eax * 4) + (G_IMGBASE + 0x1fb240U)))), where a goto L_injmp belonged; all 6 pass on the new one, in 8ms. Wired in as the 'recomp' ctest.

Re-lifted with the fix and the run is UNCHANGED, for a reason that is now measured rather than guessed: FUN_005fac10 spans 0x005fac10..0x005fb23c with 426 instructions but does NOT contain 0x005facd5, 0x005face5, 0x005facf6 or 0x005fad07 -- its own switch cases -- and those addresses belong to NO function at all. The body has holes exactly where the cases are, left by the earlier seeding and carving, and --merge cannot reabsorb them because they are no longer functions. With no instruction there, no label is emitted and L_injmp can have no case. The translator fix is correct and cannot take effect until the body is whole.
