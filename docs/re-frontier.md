...
- notes: NEXT SLOTS, with the super-call/transcribe decision already MEASURED for each -- do not re-derive it.

The engine's demand order, once display init succeeds, has been: 38 setNativeDeviceHandle, 30 open, 25 detectDriverDatabaseProperties, then **47 setRenderDestination**. It is now asking for 47.

  * slot 50 setRenderDestinationSize (0x1002ad40, ret 12) -- GUARDED: 'MOV EAX,[ESI+0x144]; TEST EAX,EAX; JZ 0x1002ae36'. Safe to igvk_super, then read the resulting state.
  * slot 47 setRenderDestination (0x1002aaf0, ret 8, 47 ins) -- UNGUARDED: 'MOV EAX,[ESI+0x144]; ... MOV ECX,[EAX]' dereferences a NULL device. Must be transcribed up to that call.
  * slot 46 setRenderDestination (0x1002ab80, ret 12, 129 ins) -- UNGUARDED at 0x1002acb2, same shape.

Both 46 and 47 select an entry from the render-destination pool at this+0x178 (index & 0xffff, stride 0x38), skip if the entry's +0x1c == 2, and end by passing the entry's +0x24 -- its surface -- to the device. For this backend there is only one destination, the swapchain, so the transcription is: run the pool selection and state stores, and instead of the device call tell igvk_device which target is bound. igvk_device has no off-screen target support yet, so anything but the back buffer must SAY so rather than silently draw to the wrong place.

The frame slots themselves (174 beginDraw, 177 clearRenderDestination, 186 setViewport, 175 endDraw) are already implemented and the host path is verified by the vk_frame_path ctest.
