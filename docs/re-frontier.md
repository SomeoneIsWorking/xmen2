...
- gap: NOT DRIVEN BY THE ENGINE: the game has still never executed a frame, so whether the engine's own order of beginDraw / clearRenderDestination / setViewport / endDraw produces the right picture is untested. It currently stops earlier, in XMen2.exe, on an indirect call to 0x005fb270 inside an SEH-protected function -- an rc-lift defect (issue #21), not a renderer one, and now the single thing between this step and its first frame. Also owed WITHIN the step: igDxVisualContext::open (slot 30) cannot succeed while createDevice checks a NULL IDirect3D8 (see notes); initDesktopDisplayFormat and initCg (the NVIDIA Cg path, C112) are not called; this+0x148 is an allocated but ZEROED D3DCAPS8, so every capability the game queries reads 'not supported'; endDraw's frame-timing branch is not reproduced (it needs an FPU return path the guest-call helper lacks) and reports itself if the engine ever enables it; clearRenderDestination does not clamp the stencil value to the target's stencil bits.
