...
- status: re-partial
- deps:
- evidence: C160 proves the PC game opens/reads a pad; C184 parses the Xbox options package; C187 records the 21 verified bindable assignments and their falsifier. Xbox `sub_00162240` directly associates each d-pad name with NEXT/PREV/INC_AGGR/DEC_AGGR; PC `FUN_00619c40`, `FUN_0061b030`, and `FUN_006281f0` recover the action rows, 42-row object, and physical codes. `test_xbox_defaults` pins every canonical tuple; `test_player_input` proves the single publisher writes them to the assigned player's master, working, and menu sets. C188 proves Xbox `sub_00163240` registers BLACK/WHITE at physical-value source indices 8/9; `sub_00163E40` expands the analog bytes and `sub_0015FD90` copies the 30-float array into the per-player controller separately from its logical mask. C189 resolves that controller: vtable `0x004A9D6C` slot `+0x10` IS `sub_0015F5B0`, returning the float at `[this + index*4 + 0x2fc]`, a 30-entry array abutting the previous-mask word at `+0x374`. Xbox `sub_00088680` and PC `FUN_0047a140` retain equivalent separate `HEALTH_ITEM`/`ENERGY_ITEM` consumption branches; C191 shows those two names are item-TYPE strings in the table at `0x0053FEBC` that no instruction references. C190 closes the literal-index search with its denominators, and C192 resolves the whole path -- manager `sub_00160E60` -> slot `+0x4c` `sub_0015F940(player)` -> controller slot `+0x10` -- and enumerates every literal physical index actually read. `tools/xbe_query.py` (I053) is the tooling for all of it.
...
- gap: Enumerate the callers of Xbox `sub_001E4210` (vtable 0x004B2600 slot 0x2c) -- the only physical reader whose index is not a literal -- and see whether any passes 8 or 9. The rest of the chain is closed with denominators (C192: 246 controller calls attributed to the 356 getController sites; literal physical indices read are 0,1,2,0xb,0xc,0x10,0x11 only. C190: no literal 8/9 at slot +0x10 anywhere. C191: neither HEALTH_ITEM nor ENERGY_ITEM is referenced by any instruction). Then port only that trigger into the retained PC Health/Energy consumer, and validate device assignment on physical hardware.
