...
- gap: The game now reaches drawing and gets past the intro movies: on a real run it presents 2400+ frames and stops on a MISSING RECOMPILED BODY (XMen2.exe 0x0045fda0), which is the discovery loop's work, not d3d8's. What EXISTS and is proved: the draw path end to end, by pixel readback rather than by call counting -- --d3d8-selftest locks a vertex buffer through the COM vtables, fills it, rasterises it into an off-screen target and checks the pixels, with a corner checked too so a shader that filled everything would fail. 53 of 97 device methods, textures, CUBE textures (six faces with separate storage, checked by the cube self-test), and vertex/index buffers with real Lock/Unlock including sub-rectangles (issue #51). NOT written: SAMPLING a cube -- the fixed-function shader has a 2D sampler, so a cube bound to the stage refuses the draw by name rather than showing the wrong face; multi-texture stages (stage 0 only); real vertex/pixel shaders (refused by name, not approximated); a depth target (the depth test is accepted and IGNORED, and says so once); triangle fans; and off-screen render targets for the engine's own use.
