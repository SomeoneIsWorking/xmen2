...
- gap: Enumerate the callers of Xbox `sub_001E4210` (vtable 0x004B2600 slot 0x2c) -- the only physical reader whose index is not a literal -- and see whether any passes 8 or 9. The rest of the chain is closed with denominators (C192: 246 controller calls attributed to the 356 getController sites; literal physical indices read are 0,1,2,0xb,0xc,0x10,0x11 only. C190: no literal 8/9 at slot +0x10 anywhere. C191: neither HEALTH_ITEM nor ENERGY_ITEM is referenced by any instruction). Then port only that trigger into the retained PC Health/Energy consumer, and capture the real controller editor showing both labels with both clicks proved on-screen.
