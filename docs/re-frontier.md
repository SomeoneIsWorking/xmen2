...
- gap: The game does not reach a draw: it stops in issue #25, upstream of rendering. What EXISTS and is proved: the draw path end to end, by pixel readback rather than by call counting -- --d3d8-selftest locks a vertex buffer through the COM vtables, fills it, rasterises it into an off-screen target and checks the pixels, with a corner checked too so a shader that filled everything would fail. 44 of 97 device methods, plus textures and vertex/index buffers with real Lock/Unlock. NOT written: multi-texture stages (stage 0 only), real vertex/pixel shaders (refused by name, not approximated), a depth target (the depth test is accepted and IGNORED, and says so once), triangle fans, and off-screen render targets for the engine's own use.
