...
SCALE, measured (C127): of the 98 device slots only FIVE are guarded and therefore super-callable (30, 34, 36, 50, 331). 78 dereference the device unguarded and must be IMPLEMENTED; 15 reach it only through a call. So the backend's 'super-call by default' design covers far less than it looked like it would, and the bulk of the remaining work is real: most of the 78 are state setters that were about to hand a value to SetRenderState, and they need a host-side state mirror that the draw path consumes. tools/device_slots.py --list now prints the guarded/UNGUARDED verdict per slot, so this does not need re-deriving.

Also owed: this+0x148 is an allocated but ZEROED D3DCAPS8, so every capability the game queries reads 'not supported'; the engine binds render destination 3, an OFF-SCREEN target this backend does not have (igvk_frame_bind_target says so at runtime); initDesktopDisplayFormat and initCg are not called and Cg is unavailable (LoadLibraryA returns NULL, which initCg handles); endDraw's frame-timing branch is not reproduced; clearRenderDestination does not clamp the stencil value.
