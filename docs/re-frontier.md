...
- status: re-partial
- deps:
- evidence: C160 proves the PC game opens/reads a pad; C184 parses the Xbox options package; C187 records the 21 verified bindable assignments and their falsifier. Xbox `sub_00162240` directly associates each d-pad name with NEXT/PREV/INC_AGGR/DEC_AGGR; PC `FUN_00619c40`, `FUN_0061b030`, `FUN_006281f0`, and `FUN_006297a0` recover the action rows, physical codes, 42-row object, and exact setter. `FUN_0061dc10`/`FUN_006188c0` prove the retained three-button editor boundary. `test_xbox_defaults` calls the shipping wrappers and proves every tuple, both labels, callback ABI, automatic custom-map preservation, explicit replacement/persistence, idempotence, and owned-row disconnect removal. Xbox `sub_00163240` constructs distinct BLACK and WHITE platform records, while Xbox `sub_00088680` and PC `FUN_0047a140` retain equivalent separate `HEALTH_ITEM`/`ENERGY_ITEM` consumption branches. The record fields' physical/action semantics and the trigger joining those two facts are not yet recovered.
...
- gap: Recover how the Xbox BLACK/WHITE platform records become action-mask bits and reach the already-retained PC Health/Energy event consumer, then port only that trigger; capture the real controller editor showing both labels and prove both clicks on-screen.
