...
- gap: NOT DRIVEN BY THE ENGINE: 0 frames presented, so whether the engine's order of beginDraw / clearRenderDestination / setViewport / endDraw produces the right picture is untested. The run no longer stops in the recompiler (C124 -- the switch-dispatch defect is fixed and the exe runs past it); it now stops on the game's OWN 'Display failed!' dialog, issue #22, which is renderer work. Note that NO unimplemented slot was dispatched before that dialog, so open (slot 30) is NOT what is being refused -- find the dialog's caller before implementing more slots. Also owed WITHIN the step: this+0x148 is an allocated but ZEROED D3DCAPS8, so every capability the game queries reads 'not supported'; initDesktopDisplayFormat and initCg (the NVIDIA Cg path, C112) are not called; the two virtual calls through this+0x534 and this+0x53c that the engine's own userInstantiate makes are not made; endDraw's frame-timing branch is not reproduced and reports itself if the engine enables it; clearRenderDestination does not clamp the stencil value to the target's stencil bits.
- notes: NEXT STEP, fully scoped so it does not need re-deriving. igDxVisualContext::open (slot 30, libIGGfx 0x1002c5c0, RET 4, hidden-pointer igStatus return) brings the display up. Its shape: if this+0x144 already exists return OK; else call vtable slot 29, then createDevice(this+0x17c); on failure return the FAIL singleton; on success call 0x1002fa60, then 0x1002ca30, then vtable slot 217 with -1, store this at the global 0x101895a8, and return OK. Only ONE of those is a problem: igDxVisualContext::createDevice at 0x1002cb80 (RET 4, bool in AL). Its very first act is 'if (this+0x140 == 0) return false', and this+0x140 is the IDirect3D8 this backend deliberately never creates -- so open cannot currently succeed. Two ways, and it is a real choice: (a) implement slot 30 as a transcription of open with createDevice replaced by 'our Vulkan device already exists' -- six named calls, explicit, no build plumbing, a small amount of re-derivation; (b) native-override createDevice itself (src/native/overrides.json, the mechanism already used for XMen2.exe 0x00617480) so it reports success and sets the fields the engine's version sets, then SUPER-CALL open unchanged -- zero re-derivation, and it generalises to every other non-virtual D3D-creating helper, but needs the override mechanism pointed at a libIG*.dll rather than the exe and gating on --vk. (b) is the better shape and matches the super-call design the backend already uses; (a) is the smaller change. Either way createDevice is 1106 bytes and also writes presentation parameters that later code reads, so 'return true and do nothing' is NOT sufficient -- read it before replacing it.
