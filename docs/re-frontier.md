...
- gap: No resources and no draw path. The engine stops at IDirect3DDevice8::GetBackBuffer -- it wants the back buffer and the depth/stencil as IDirect3DSurface8 objects to use as its render destinations, and permissive mode cannot walk past that because it then dereferences the NULL it was handed. Owed, in the order the engine asks: surfaces (GetBackBuffer, GetDepthStencilSurface, SetRenderTarget), then textures and vertex/index buffers with real Lock/Unlock, then the draw calls, then the state mirror actually programming a pipeline. 29 of 97 device methods are written; every unwritten one reports its interface and method NAME. The caps block (src/d3d8/d3d8_caps.c) is a DECLARED profile, not a measured one -- see its file comment.
