...
| Touch play | Decide from the live event stream whether touch is the device in use, then own the title's safe-area-aware virtual pad: action vocabulary, layout, multi-touch routing, cancellation, on-screen feedback, direct portrait selection, and touch-only relocation of the retained retail HUD. Built and shipped on EVERY platform; `input.touch_controls` (OFF/AUTO/ALWAYS) forces either end | `src/input/touch_source.{c,h}`, `src/input/touch_controls.{cpp,h}`, `src/input/touch_runtime.{cpp,h}`, `src/input/gameplay_control.{c,h}`, `src/presentation/touch_layout.{c,h}`, `src/native/touch_hud_runtime.{c,h}`, `src/ui/touch_document.{cpp,h}`, `assets/ui/touch_controls.rcss`, shared `lucent::touch::Router` | `x2_touch_source_note`, `x2_touch_runtime_event`, `x2_touch_runtime_active`, `x2_touch_runtime_overlay_visible`, `touch_document_update`, overrides at `FUN_005a43d0`/`FUN_005a3320` | [Touch play](touch-play.md) |
