...
| Web target portability measurement | Measure which engine translation units compile to wasm32 and name the ones that do not with the compiler error that explains them; refuse a result that measured nothing. There is NO web product: this owns the measurement while the target is blocked, not a build | `tools/wasm_portability.py`, `tools/ci.py` (`wasm-portability`), `tools/ci_support.py` (`web-wasm` target), `.github/workflows/asset-free.yml` | `wasm_portability.py:main`, `ci.py:wasm_portability` | [Web release](web-release.md) |
