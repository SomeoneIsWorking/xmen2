...
| Which recompiled bodies were ever entered, and in what order | `src/native/x86rt_native.c` (`-DX2_NATIVE_REACHED=ON`, `X2_REACHED`) | **verified** — I021; a SET with first-entry ordering, not a history, because the ring evicts and so cannot answer "was this ever called". Reports the distinct-entry count as its denominator and probes an address only against modules whose LINKED range contains it. `X2_REACHED_SELFTEST=1` checks reached, never-reached AND the ordering, and exits 4 if any fails. Answered the question issue #14 turned on |
