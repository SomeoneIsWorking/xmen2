...
| Android setup and release | Own the no-terminal setup screen and `GameImportNotification` wording; Lucent owns Activity/SAF staging and foreground import service lifetime; consume the pinned `shared/android-port` prefix, JDK selection and package checks; compose title validation, signed APK, launcher icon, and named-device performance evidence collection. It CONSUMES touch play, it does not own it | `android/`, `src/native/android_bridge.{cpp,h}`, `src/native/install_validation.{cpp,h}`, `tools/build_android.py`, `tools/android_qualify.py`, CMake Android branch | `XMen2SetupActivity`, `XMen2GameActivity`, `x2_install_validate_executable`, `tools/android_qualify.py:main`, `tools/build_android.py:main` | [Android release](android-release.md) |
