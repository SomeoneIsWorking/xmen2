...
| MSVCR71 on libc | `src/native/crt.c` | **partial** — C097; the varargs family (printf/sprintf/_snprintf/vsprintf/_vsnprintf/vprintf) is now implemented by a format walker over the guest stack, since a va_list on x86-32 cdecl IS a guest stack pointer. MSVC's truncation semantics are applied explicitly; an unimplemented conversion refuses by name. C++ EH still stops by name — it needs a real SEH unwinder |
