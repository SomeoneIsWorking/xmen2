...
| Android setup and release | Own the no-terminal setup Activity, SAF permissions and app-private staging; compose the ARM64 shared target, signed APK, launcher icon, title-authored touch feedback, and touch-only relocation of the retained retail HUD | `android/`, `src/native/android_bridge.{cpp,h}`, `src/input/touch_controls.{cpp,h}`, `src/input/touch_runtime.{cpp,h}`, `src/presentation/touch_hud_layout.{c,h}`, `src/native/touch_hud_runtime.{c,h}`, `src/ui/touch_document.{cpp,h}`, `assets/ui/touch_controls.rcss`, `tools/build_android.py`, `tools/build_android_deps.py`, CMake Android branch | `XMen2SetupActivity`, `XMen2GameActivity`, `x2_touch_runtime_event`, overrides at `FUN_005a43d0`/`FUN_005a3320`, `touch_document_update`, `tools/build_android.py:main` | [Android release](android-release.md) |
