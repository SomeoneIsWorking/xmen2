...
| D3D8 selector evidence | Passively correlate runtime texture dimensions/fingerprints with pre-build draw geometry and accepted/refused lowering results, without asserting asset or scene identity | `src/d3d8/d3d8_selector_probe.{c,h}`, `src/d3d8/d3d8_selector_probe_bridge.c`, `tools/selector_probe.py` | `d3d8_selector_probe_build_request`, `tools/selector_probe.py` | — |
...
| Presentation policy | Window settings, transactional live logical-resolution changes, aspect fit, display-mode publication, and boot blackout | `src/presentation/`, `src/native/display_mode_runtime.c` | `live_resolution.c`, `display_mode_seed.c`, `x2_override_display_settings_load` | — |
...
| Win32 window events and mouse | Translate SDL window/mouse events into the ordered Win32 queue consumed by the retained Alchemy WndProc, map through logical-output aspect fit, and arbitrate the one visible cursor | `src/native/win32_events.c`, `src/native/win32_mouse.c`, `src/native/win32_sdl.c` | `imp_USER32_PeekMessageA`, `imp_USER32_DispatchMessageA`, `x2_win32_mouse_map_point` | — |
...
- **A host mouse/window event** → SDL translation and guest message dispatch
  belong in `src/native/win32_events.c`; pure queue, coordinate, and cursor
  policy belong in `src/native/win32_mouse.c`.
