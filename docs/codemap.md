...
| Browser product composition | Shared-prefix build, native worker entry, title install/launch page, explicit release resource map, and asset-free CI artifact; sibling `pages` owns public deployment | `tools/build_web.py`, `cmake/WebDependencies.cmake`, `cmake/WebTarget.cmake`, `src/web/web_main.cpp`, `web/`, `.github/workflows/browser-package.yml` | `build_web.py`, browser `main` | [Web release](web-release.md) |
