...
| Host GPU | SDL_GPU device, uploads, draws, capture, composition, frame submission, prompt glyphs, and shadow pass | `src/gpu/` | `gpu_device.c`, `gpu_draw.c` | [Text and prompts](RE/text.md), [Shadows](RE/shadows.md) |
| Input policy | Binding rows, device identity/admission, player association, participation, probing lifecycle, and exact recording | `src/input/` | `player_input.c`, `input_record.c` | [Co-op participation](RE/co_op_participation.md) |
| Media decode | SFD demux, video decode, ADX decode, timing, drain, and probe policy | `src/media/` | `fmv_player.c` | [FMV](RE/fmv.md) |
| Native host and bridges | PE/runtime composition, POSIX Win32 services, title and engine overrides, live control, boot, input publication, and diagnostics | `src/native/` | `x2native.c`, `x86rt_native.c` | [Boot](RE/boot.md) |
| Oracle records | Shared binary record ABI for stock-proxy and native traces | `src/oracle/` | `probe_rec.h` | — |
| Presentation policy | Window settings, aspect fit, display-mode publication, and boot blackout | `src/presentation/` | `window_settings.c`, `aspect_fit.c` | — |
...
| Save model | Retail save directory, catalog, Continue policy, autosave policy/format/storage, and trace model | `src/save/` | `save_directory.c`, `save_catalog.c` | [Boot](RE/boot.md) |
...
| Save runtime bridges | Connect guest save/autosave/Continue calls to the pure save owners | `src/save/`, `src/native/autosave_runtime.c`, `src/native/continue_runtime.c`, `src/native/save_trace_runtime.c` | override registration in the native bridge files | [Boot](RE/boot.md) |
...
| Presentation and capture | Own logical-backbuffer composition, aspect fit, swapchain lifetime, retained-frame readback, and control-channel screenshots | `src/presentation/`, `src/gpu/gpu_present.c`, `src/gpu/gpu_capture.c`, `src/native/win32_sdl.c`, `src/native/control_screenshot.c` | `gpu_present_composite`, `control_screenshot_request` | — |
| Live control and recording | Publish live-session discovery, accept loopback control requests, merge scripted input on guest polls, report state, and record delivered input | `src/native/control.c`, `src/native/control_status.c`, `src/native/control_query.c`, `src/native/live_session.c`, `src/input/input_record.c`, `tools/x2ctl.py` | `control_start`, `tools/x2ctl.py:main` | — |
| In-game cutscene player | Complete a control-lock epoch through causally owned timed events and BehavEd fibers; use deterministic conversations only as subordinate payloads | `src/native/cutscene_player.c`, `src/native/cutscene_event_player.c`, `src/native/behaved_player.c`, `src/native/conversation_player.c`, `src/native/cutscene_skip_publication.c` | override registration in each player; action-20 edge in `cutscene_player.c` | [Cutscene player](RE/cutscene_player.md) |
...
| Port settings overlay | Compose persistent settings, controller assignment rows, and pause-menu command integration | `src/ui/`, `src/config/`, `src/native/options_menu.c`, `tools/make_port_pause_menu.py`, `tools/prepare_native_assets.py`, `assets/ui/settings.rcss` | `rmlui_ui_init`, override registration in `options_menu.c` | [Prior art](prior-art.md) |
...
- **A reusable matrix operation** → `src/gpu/gpu_matrix.c`; engine matrix
  ...
  `src/d3d8/`.
- **A setting or stored assignment** → `src/config/`; window-mode application
  belongs in `src/presentation/`; UI editing belongs in `src/ui/`.
