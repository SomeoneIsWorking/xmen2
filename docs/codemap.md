...
| Browser touch reachability | Claim the browser's own scroll, pinch, callout and overscroll gestures for the game canvas so a contact reaches the pad at all, and measure that it does | `web/app.mjs`, `web/style.css`, shared `web-port` `platforms/web/canvas.mjs`, `tools/web_touch_probe.py` | `claimCanvasGestures`, `tools/web_touch_probe.py:main` | [Touch play](touch-play.md) |
...
| Touch routing | SDL contact acquisition on every platform, the window and viewport both the zones and the relocated HUD lay out from, and the one decision that follows a contact: a drawn control under it is a pad press, no drawn control under it is the retail pointer | `src/input/touch_runtime.{h,cpp}` | `x2::input::TouchRuntime`, `x2_touch_runtime_event`, `x2_touch_runtime_inject` | [Touch play](touch-play.md) |
