...
| Native bounding-box frustum test | Native overrides for libIGSg.dll's clip-space box corners (`0x10047570`) and their clip-code classification (`0x100478e0`), exact to the guest's x87 operation order and spills on an x87 host and the guest body elsewhere and for the guard-band case; `sg.box_cull_verify` re-runs the guest body and aborts on any mismatch | `src/native/box_cull.{c,h}`, `src/native/box_cull_override.c` | `box_cull_corners`, `box_cull_classify`, `x2_override_10047570`, `x2_override_100478e0` | [issue #141](issues/0141-engine-jit-menu-in-game-throughput-is-capped-35.md) |
