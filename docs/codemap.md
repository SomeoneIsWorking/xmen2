...
| Browser product composition | Shared web-port host storage/isolation, native worker entry, OPFS unpublished extraction and ready marker, browser log routing, title install/launch page with a Dead Zone test boot, explicit release resource map, and asset-free CI artifact; sibling `pages` owns public deployment | shared `web-port` runtime, `tools/build_web.py`, `cmake/WebDependencies.cmake`, `cmake/WebTarget.cmake`, `src/web/web_main.cpp`, `src/web/browser_log.{cpp,hpp}`, `src/native/install_archive.{cpp,h}`, `web/`, `.github/workflows/browser-package.yml` | `build_web.py`, browser `main` | [Web release](web-release.md) |
