...
| Browser run measurement | Read a live browser run from outside the page: a dependency-free CDP client, a sampling CPU profile of the guest worker with a denominator, and a full console recorder for the heartbeat's own evidence, which WebLua's small buffer loses. Attaches to the Chrome WebLua already owns; launches and drives nothing | `tools/cdp_client.py`, `tools/web_profile.py`, `tools/web_console.py` | `web_profile.py:main`, `web_console.py:main` | [issue #155](issues/0155-the-stall-diagnostic-never-stopped-printing.md) |
