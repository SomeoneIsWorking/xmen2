...
| Android setup and release | Own the no-terminal setup Activity, SAF permissions and app-private staging; compose the ARM64 shared target and APK without host dependency leakage | `android/`, `src/native/android_bridge.{cpp,h}`, `src/input/touch_runtime.cpp`, `tools/build_android.py`, `tools/build_android_deps.py`, CMake Android branch | `XMen2SetupActivity`, `XMen2GameActivity`, `tools/build_android.py:main` | [Android release](android-release.md) |
