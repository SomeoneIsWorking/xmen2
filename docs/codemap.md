...
| Host GPU | SDL_GPU device, deterministic frame initialization, uploads, draws, capture, composition, frame submission, bounded exact frame timing, prompt glyphs, and shadow pass | `src/gpu/` | `gpu_device.c`, `gpu_depth.{c,h}`, `gpu_draw.c`, `gpu_frame_timing.{c,h}`, `gpu_frame_timing_report.{c,h}` | [Text and prompts](RE/text.md), [Shadows](RE/shadows.md) |
...
| Guest preemption quantum | How many boundary crossings the running guest thread keeps the guest lock before `guest_quantum` offers it up (`X2_QUANTUM`; 0 is the no-preemption control for issue #57) | `src/native/threads_quantum.c` | `guest_quantum_from_env`, `guest_quantum_size` | — |
...
| Guest address space | One owner for every host: exact Win32 4 KiB page state, an identity or reserved host arena where the VM enforces it, and a flat window plus a byte-per-page permission table where there is no VM. Tells the execution owner when mapped bytes change | `src/native/guest_memory.c`, `src/native/guest_memory.h`, `src/native/platform_mman.h` | `guest_memory_init`, `guest_memory_pointer`, `guest_memory_map_fixed`, `guest_memory_window`, `guest_memory_set_remap_observer` | — |
| Guest address-space layout | Where the image, relocated modules, guest reservations, the runtime arena and file views live, and the top the browser window must cover | `src/native/guest_layout.h` | `GUEST_MODULE_LO`, `GUEST_RESERVE_LO`, `GUEST_HEAP_BASE`, `GUEST_LAYOUT_LIMIT` | — |
...
| Native bounding-box frustum test | Native overrides for libIGSg.dll's clip-space box corners (`0x10047570`), their clip-code classification (`0x100478e0`) and the box-test driver that calls both (`0x10047470`), exact to the guest's x87 operation order and spills on an x87 host and the guest body elsewhere and for the guard-band case; `sg.box_cull_verify` re-runs the guest body and aborts on any mismatch | `src/native/box_cull.{c,h}`, `src/native/box_cull_override.c` | `box_cull_corners`, `box_cull_classify`, `x2_override_10047470`, `x2_override_10047570`, `x2_override_100478e0` | [issue #141](issues/0141-engine-jit-menu-in-game-throughput-is-capped-35.md) |
...
| Native igMatrix44f::multiply | Native override for libIGMath.dll's 4x4 multiply (`0x10019520`), exact to the guest's per-row x87 summation order on an x87 host and the guest body elsewhere; `math.matrix_verify` re-runs the guest body and aborts on any mismatch. `x87_exact` is the shared test for when host `long double` is the guest's x87 arithmetic | `src/native/ig_matrix.{c,h}`, `src/native/ig_matrix_override.c`, `src/native/x87_exact.{c,h}` | `ig_matrix44_multiply`, `x2_override_10019520`, `x87_exact_for_guest` | [issue #141](issues/0141-engine-jit-menu-in-game-throughput-is-capped-35.md) |
...
| Browser run measurement | Read a live browser run from outside the page: a dependency-free CDP client, a sampling CPU profile of the guest worker with a denominator, and a full console recorder for the heartbeat's own evidence, which WebLua's small buffer loses, and the frame-rate table that reads, reporting the sustained plateau beside the median so a contended host cannot pass as a slow build and a steady rate counted once across that plateau so a change smaller than one frame per window is still resolvable, and the hot-block differencer that turns the heartbeat's cumulative histogram into per-window shares for an address range, refusing to guess at a row that entered the printed forty between two snapshots. Attaches to the Chrome WebLua already owns; launches and drives nothing | `tools/cdp_client.py`, `tools/web_profile.py`, `tools/web_console.py`, `tools/web_presents.py`, `tools/web_hotblocks.py` | `web_profile.py:main`, `web_console.py:main`, `web_presents.py:report`, `web_hotblocks.py:report` | [issue #155](issues/0155-the-stall-diagnostic-never-stopped-printing.md) |
