...
| Browser run measurement | Read a live browser run from outside the page: a dependency-free CDP client, a sampling CPU profile of the guest worker with a denominator, and a full console recorder for the heartbeat's own evidence, which WebLua's small buffer loses, and the frame-rate table that reads, reporting the sustained plateau beside the median so a contended host cannot pass as a slow build. Attaches to the Chrome WebLua already owns; launches and drives nothing | `tools/cdp_client.py`, `tools/web_profile.py`, `tools/web_console.py`, `tools/web_presents.py` | `web_profile.py:main`, `web_console.py:main`, `web_presents.py:report` | [issue #155](issues/0155-the-stall-diagnostic-never-stopped-printing.md) |
