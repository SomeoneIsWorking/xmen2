...
| x86-32 → C translator | `tools/recomp.py` | **partial** — XMen2.exe 13,426/13,501 functions, 98.7% of instructions in translated functions. The 75 blocked are SSE/MMX and 3DNow! (PFMUL/PFADD/PI2FD/PSWAPD — an AMD path the game selects on CPUID), plus FPTAN, PUSHAD, RCR. `emit --split N` chunks the output: the exe is 2.05M lines and cost 94s / 4.8 GB as one TU |
