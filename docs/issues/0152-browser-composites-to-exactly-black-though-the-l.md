...
### Update (2026-09-18, next session): "still loading" is REFUTED by a same-route native comparison; the defect is upstream of compositing, in the real fixed-function draw path -- and it has ZERO selftest coverage on any backend

The previous update's tentative "maybe the game just hasn't drawn a real frame yet" theory does not survive a same-route comparison against native. `scratch/web/native-window-luma2.log` is a `present_luma` capture from a PRIOR session, and its boot log confirms it used the exact same route as every browser sample in this issue: `X2_BOOT_MAP: the boot's intro script is replaced by the retail startFirstMission initializer, then a direct load of "act1/deadzone/deadzone1"`. Its readings, at presents 41/81/121/161:

```
composed mean 38.3 max 197 nonblack 54.9% | scene read mean 38.3 max 197 nonblack 54.9% (frame 1920x1080, scene 1920x1080, 2 draw(s) so far)
```

Native reaches real, stable, half-the-frame-lit visible content (`nonblack 54.9%`) with only **2 draws**, and `composed == scene` exactly -- compositing is a correct pass-through of real content, as this issue already proved it is in-browser too. The browser's own long-running sample (this session, same route, same present_luma instrument) had, by the time this comparison was made, reached **3212 real draws over 295 wall-seconds, `refused 0`** (`[HB]` heartbeat), yet `scene read mean 0.1 max 191 nonblack 0.1%` -- unchanged from every earlier sample in this issue. Native needs 2 draws to produce visible content on this exact map; browser has done over 1500x that many draws and produced none. "The level hasn't loaded yet" cannot explain a gap of that size on an identical boot route -- this rules out the loading-speed theory from the prior update and returns the defect to being a real, distinct rendering bug, just one level upstream of where this issue previously looked: not the composite/capture mechanism (proven correct three separate ways above), but the actual fixed-function DRAW path that fills `g_scene` in the first place.

Went looking for existing coverage of that path and found none: `grep` for `gpu_draw_submit`/`d3d8_fixed` across every `src/gpu/*selftest*.c` returns nothing. Every selftest this project has for the GPU backend (`gpu_selftest.c`, `gpu_multistage_selftest.c`, `gpu_present_selftest.c`, `gpu_texture_selftest.c`, `gpu_upload_selftest.c`, `gpu_shadow_selftest.c`, `--vk-selftest`) exercises presentation, capture, texture upload, or shadow rendering -- never the real fixed-function vertex/fragment shader pair (`src/gpu/shaders/d3d8_fixed.vert`/`.frag`) through `gpu_draw.c`'s actual draw-submission path with a populated `VertexState` uniform buffer (MVP matrix, lighting, texgen, the works). The one path this whole issue is now pointing at is the one path nobody has ever put a pixel-correctness check on, on any backend.
